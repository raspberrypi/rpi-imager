...
_fetch_pool_deb_keyring() {
	...
	_gpg="$_tmp/root/usr/share/keyrings/${_pkg}.gpg"
	if [ ! -f "$_gpg" ]; then
		_gpg="$_tmp/root/usr/share/keyrings/${_pkg}.pgp"
	fi
	if [ ! -f "$_gpg" ]; then
		_gpg=$(find "$_tmp/root/usr/share/keyrings" -maxdepth 1 -type f -name "${_pkg}.*" 2>/dev/null | head -1)
	fi
	[ -n "$_gpg" ] || {
...
ensure_keyring() {
	...
	case "$_label" in
		debian-archive-keyring)
			if [ -f "$_host" ] && debian_keyring_has_trixie_keys "$_host"; then
				copy_or_use "$_dest" "$_host" && return 0
			elif [ -f "$_host" ]; then
				echo "fetch-archive-keyrings: host $_host lacks trixie keys; downloading fresh keyring" >&2
			fi
			;;
		*)
			copy_or_use "$_dest" "$_host" && return 0
...
fetch_debian() {
	ensure_keyring debian-archive-keyring \
		...
		"$KEYRING_CACHE/debian-archive-keyring.gpg" \
		"$DEBIAN_MIRROR" "$CHROOT_DIST" debian-archive-keyring \
		pool/main/d/debian-archive-keyring/debian-archive-keyring_2025.1_all.deb
}

fetch_raspbian() {
	_rasp_mirror=${RASPBIAN_MIRROR%/}
	...
	ensure_keyring raspbian-archive-keyring \
		...
		"$KEYRING_CACHE/raspbian-archive-keyring.gpg" \
		"$_rasp_mirror" "$CHROOT_DIST" raspbian-archive-keyring \
