...
DownloadThread::~DownloadThread()
{
    ...
    // Wait for any pending hash computation unconditionally, rather than
    // asking a flag. The future is assigned before _hasPendingHash is set,
    // so a destructor landing between the two skipped the wait and left a
    // pooled task running against a destroyed object -- which is what
    // ThreadSanitizer reports as a race on this object's QWaitCondition,
    // torn down while a QThreadPool thread still holds it. Waiting on a
    // future that never ran returns at once, so there is nothing to gate.
...
size_t DownloadThread::_writeFile(const char *buf, size_t len, WriteCompleteCallback onComplete)
{
    ...
    // Pipelined hash computation: wait for the PREVIOUS hash before starting
    // this one. Unconditionally, because the line below overwrites the future
    // -- and dropping the only handle on a task still running leaves nothing
    // able to wait for it. ThreadSanitizer catches the consequence as a
    // use-after-free: the hash object destroyed while a pooled _hashData is
    // still writing into it. Asking _hasPendingHash first was the hole, since
    // the flag is set after the future is assigned.
...
int DownloadThread::safeAsyncQueueDepth(int requestedDepth, qint64 availableMemMB,
                                        size_t writeBlockSize)
{
    // Hard cap at 256 to prevent resource exhaustion
    const int maxDepth = 256;
    requestedDepth = (requestedDepth < 1) ? 1
                                          : ((requestedDepth > maxDepth) ? maxDepth : requestedDepth);

    if (writeBlockSize == 0 || availableMemMB <= 0)
...
{
    const int requestedDepth = safeAsyncQueueDepth(
        depth,
        SystemMemoryManager::instance().getAvailableMemoryMB(),
        SystemMemoryManager::instance().getOptimalWriteBufferSize());

    _debugAsyncQueueDepth = requestedDepth;
    const size_t writeBlockSize = SystemMemoryManager::instance().getOptimalWriteBufferSize();
    qDebug() << "DownloadThread: Async queue depth set to" << _debugAsyncQueueDepth
             << "(estimated memory:"
             << ((_debugAsyncQueueDepth * writeBlockSize) / (1024 * 1024)) << "MB)";
}
