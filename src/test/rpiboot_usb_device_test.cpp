...
// Transfers against a device that went away mid-sideload.
//
// The cable coming out during a bootcode push is the ordinary failure, and
// openDevice() refusing a stale device (above) only covers the case where
// nothing was open yet. Here the handle is real and the device behind it
// is gone, which is the state every transfer has a guard for and none of
// them had ever been in: bulkWrite's retry loop decides whether the error
// is worth retrying, and bulkRead has to hand the libusb code back rather
// than report a short read as a successful one.
TEST_CASE("Transfers to a Compute Module that was unplugged report the error",
          "[rpiboot][usbdevice]")
{
    REQUIRE_EMULATOR();
    ...
    while (steady_clock::now() < deadline && !ctx.scanBootDevices().empty())
        std::this_thread::sleep_for(milliseconds(100));
    REQUIRE(ctx.scanBootDevices().empty());

    const std::vector<uint8_t> payload(64, 0x5A);
    const int written = transport->bulkWrite(0x01, payload, 500);
    INFO("bulkWrite returned " << written);
    // Negative, and not silently reported as the byte count asked for: a
    // caller that saw 64 here would carry on to the next block of a
    // bootcode that never arrived.
    CHECK(written < 0);

    std::vector<uint8_t> buffer(64, 0);
    const int read = transport->bulkRead(0x82, buffer, 500);
    INFO("bulkRead returned " << read);
    CHECK(read < 0);

    // The control channel too, which is how the protocol asks for sizes.
    CHECK_FALSE(transport->controlTransfer(0x40, 0, 0, 0, payload, 500));
}
