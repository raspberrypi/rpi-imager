...
// A partition is a window onto the whole device, and the only thing stopping
// one partition's I/O from landing in another is these bounds checks. They
// are written as (size > end - offset) rather than (offset + size > end) so
// an offset near UINT64_MAX cannot wrap past the comparison, which is why a
...
TEST_CASE("A partition refuses I/O outside its own window", "[devicewrapper]")
{
    ...
    // A 1 MB window starting 1 MB in, so there is device either side of it.
    constexpr quint64 kStart = 1024 * 1024;
    constexpr quint64 kLen = 1024 * 1024;
    DeviceWrapperPartition part(dw, kStart, kLen);

    QByteArray buf(512, '\0');

    SECTION("a read may not run past the end")
    {
        part.seek(static_cast<qint64>(kLen) - 256);
        CHECK_THROWS_AS(part.read(buf.data(), 512), std::runtime_error);
    }

    SECTION("a read of negative size is refused, not wrapped")
    {
        CHECK_THROWS_AS(part.read(buf.data(), -1), std::runtime_error);
    }

    SECTION("a write may not run past the end")
    {
        part.seek(static_cast<qint64>(kLen) - 256);
        CHECK_THROWS_AS(part.write(buf.constData(), 512), std::runtime_error);
    }

    SECTION("a write of negative size is refused")
    {
        CHECK_THROWS_AS(part.write(buf.constData(), -1), std::runtime_error);
    }

    SECTION("a seek past the end is refused")
    {
        CHECK_THROWS_AS(part.seek(static_cast<qint64>(kLen) + 1), std::runtime_error);
    }

    SECTION("a negative seek is refused")
    {
        CHECK_THROWS_AS(part.seek(-1), std::runtime_error);
    }

    SECTION("a seek inside the window is reported relative to its start")
    {
        part.seek(4096);
        CHECK(part.pos() == 4096);
        // The last byte is a legal destination; the end itself is too.
