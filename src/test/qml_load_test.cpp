 ...
 *
 * The application registers its C++ types through a generated function that
 * exists only in the app target, so this registers them by hand and loads the
 * QML out of the source tree. That keeps the test independent of the build's
 * resource layout, at the cost of having to keep this list in step with the
 * QML_ELEMENT declarations -- which is itself worth catching, since a type
 * added to the UI and not registered is a UI that does not start.
 */
