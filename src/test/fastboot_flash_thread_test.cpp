...
public:
    ...
    using FastbootFlashThread::registerWithConnect;
    using FastbootFlashThread::setConnectRegistration;
...
// ══════════════════════════════════════════════════════════════
// Raspberry Pi Connect registration
// ══════════════════════════════════════════════════════════════
//
// This step runs while the board is still in fastboot mode, because that is
// the only time its public key can be queried and signed by the firmware
// crypto engine. It was unreachable in a test: the registrar takes an
// injectable base URL, but the caller never passed one through, so the only
// server it could ever talk to was the live Connect API.

namespace {

const char *kConnectDevicePem =
    "-----BEGIN PUBLIC KEY-----\n"
...
// A localhost stand-in for the Connect management API, answering every POST
// with a chosen status and body.
class ConnectApiStub
{
public:
    ConnectApiStub(int status, const QByteArray &body)
        : _hits(QDir::temp().filePath(QStringLiteral("rpi-imager-connect-%1")
    ...
    // How many requests actually arrived here. Without this the test passes
    // whether the registrar talked to the stub or to the live Connect API,
    ...
    int hits() const
    {
        ...
        if (!f.open(QIODevice::ReadOnly))
            return 0;
        return static_cast<int>(f.readAll().size());
    }

    ConnectApiStub(const ConnectApiStub &) = delete;
    ConnectApiStub &operator=(const ConnectApiStub &) = delete;

    bool isRunning() const { return _port > 0; }
    QString baseUrl() const { return QStringLiteral("http://127.0.0.1:%1").arg(_port); }
...
// product and serialno are asked for first, to fill the Connect description
// field; then the registrar asks the device for its key and a signature.
void queueConnectDevice(MockUsbTransport &mock)
{
    mock.queueBulkReadResponse(okay("Raspberry Pi 5 Model B"));   // getvar:product
    mock.queueBulkReadResponse(okay("10000000abcdef01"));         // getvar:serialno
    mock.queueBulkReadResponse(okay(kConnectDevicePem));          // getvar:public-key
    mock.queueBulkReadResponse(
        okay("connect-signature:" + std::string(512, 'a')));      // sign request
}
...
TEST_CASE("A flashed device is registered with Connect", "[fastboot][connect]")
{
    ...
    TestableFlashThread t{QUrl(QStringLiteral("https://example.invalid/os.img.xz")),
                          QStringLiteral("nvme0n1")};
    t.setConnectRegistration(QStringLiteral("rpck_not_a_real_key"),
                             QStringLiteral("imager"), api.baseUrl());

    MockUsbTransport mock;
    mock.setOpen(true);
    queueConnectDevice(mock);

    fastboot::FastbootProtocol fb;
    CHECK_NOTHROW(t.registerWithConnect(fb, mock));

    // The identifiers were read off the device rather than invented, and the
    // key exchange happened: four commands go out.
    const QStringList cmds = commandsSent(mock);
    INFO("commands: " << cmds.join(QStringLiteral(" | ")).toStdString());
    CHECK(cmds.filter(QStringLiteral("product")).size() == 1);
    CHECK(cmds.filter(QStringLiteral("serialno")).size() == 1);
    CHECK(mock.capturedBulkWrites().size() >= 4);

    // And the request went to the stub, not to the live Connect API.
    CHECK(api.hits() > 0);
}
...
TEST_CASE("A Connect registration that fails does not fail the flash", "[fastboot][connect]")
{
    ...
    // not turn a completed write into a reported failure.
    ConnectApiStub api(500, R"({"error":"internal"})");
    if (!api.isRunning())
        SKIP("the local API server did not start");
...
TEST_CASE("With no Connect key the device is not registered", "[fastboot][connect]")
{
    // No key configured is the default, and it must send nothing at all --
    // not an unauthenticated request to the live API.
    TestableFlashThread t{QUrl(QStringLiteral("https://example.invalid/os.img.xz")),
                          QStringLiteral("nvme0n1")};
