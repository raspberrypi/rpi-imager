...
// A server that serves one body with a fixed ETag and honours a
// conditional GET, which SimpleHTTPRequestHandler does not.
//
// FirmwareManager keeps an ".etag" sidecar next to each cached firmware file
// and sends If-None-Match on the next run, so that repeat sessions cost a
// couple of hundred bytes rather than a firmware bundle. Nothing could reach
// that: the 304 arm, the header it sends to provoke one, and the sidecar read
// that supplies it were all unreached, and a server that always answers 200
// cannot tell a working conditional request from one that is not being sent.
class ConditionalHttpServer
{
public:
    ConditionalHttpServer(const QByteArray &body, const QByteArray &etag)
    {
        static const char *kScript =
    ...
    ~ConditionalHttpServer()
    {
        _process.kill();
        _process.waitForFinished(5000);
    }

    ConditionalHttpServer(const ConditionalHttpServer &) = delete;
    ConditionalHttpServer &operator=(const ConditionalHttpServer &) = delete;
...
// A server that delivers the first part of a real file and then slows to
// a trickle without closing the connection.
class PartialThenSilentHttpServer
{
public:
    PartialThenSilentHttpServer(const QString &file, qint64 prefixBytes,
                                qint64 announcedBytes)
    {
        static const char *kScript =
            "import http.server, socketserver, sys, time\n"
            "path = sys.argv[1]\n"
            "prefix = int(sys.argv[2])\n"
            "announced = int(sys.argv[3])\n"
            "class H(http.server.BaseHTTPRequestHandler):\n"
            "    def log_message(self, *a): pass\n"
            "    def do_GET(self):\n"
            "        self.send_response(200)\n"
            ...
            "            self.wfile.write(f.read(prefix))\n"
            "        self.wfile.flush()\n"
            // A trickle rather than silence. Stopping dead makes curl call
            // the transfer partial and hand it to the resume path, which
            // reports a server that will not do byte ranges -- the ring
            // buffer never gets its turn. One byte every two seconds keeps
            // the connection healthy while the decompressor waits on a slot
            // that will not fill, which is the mirror-slowed-to-nothing case
            // the buffer's stall timeout exists for.
            "        while True:\n"
            "            time.sleep(2)\n"
            "            try:\n"
    ...
    ~PartialThenSilentHttpServer()
    {
        _process.kill();
        _process.waitForFinished(5000);
    }

    PartialThenSilentHttpServer(const PartialThenSilentHttpServer &) = delete;
    PartialThenSilentHttpServer &operator=(const PartialThenSilentHttpServer &) = delete;
