...
TEST_CASE("A signing key can be handed to the bootstrap before it starts",
          "[rpiboot-thread]")
{
    // A secure-boot board will not accept an unsigned fastboot gadget, so the
    // key has to be in place before the thread runs -- there is no second
    // chance once the device has been told to boot. Setting it is the whole
    // of the contract here: the thread reads it when it builds the sideload.
    TestableRpibootThread t{chosenDevice(), rpiboot::SideloadMode::Fastboot};

    // Accepted before the run, and accepted twice: the wizard sets it again
    // when the user picks a different key without leaving the page.
    CHECK_NOTHROW(t.setSignFastbootGadgetKey(QStringLiteral("/keys/first.pem")));
    CHECK_NOTHROW(t.setSignFastbootGadgetKey(QStringLiteral("/keys/second.pem")));
    // And cleared again, which is what unticking the option does.
    CHECK_NOTHROW(t.setSignFastbootGadgetKey(QString()));
}

// scanRpibootDevices() is what turns a board sitting in rpiboot mode into a
// row the picker can show. portPathToString() had tests; the scan around it
// had none, so nothing checked that a device on the bus becomes a descriptor
// carrying the fields the UI keys on -- or that a bus which throws mid-scan
// comes back empty rather than propagating out into the poll thread.
TEST_CASE("A board in rpiboot mode becomes a device the picker can show", "[rpiboot][scan]")
{
    FakeBus bus;
    bus.bootDevices = { device(1, 9, {2, 4}) };
    BusView view(bus);

    const auto found = rpiboot::scanRpibootDevices(view);

    REQUIRE(found.size() == 1);
    const auto &d = found.front();
    CHECK(d.isRpiboot);
    // On the USB bus, removable, and not a block device yet -- the picker
    // uses all three to decide how to draw the row and what it may offer.
    CHECK(d.isUSB);
    CHECK(d.isRemovable);
    CHECK(d.size == 0);
    CHECK_FALSE(d.isSCSI);
    CHECK_FALSE(d.isCard);
    // The port path is how two boards on different ports stay distinct.
    CHECK(d.usbPortPath == std::vector<uint8_t>{2, 4});
    CHECK_FALSE(d.rpibootChipName.empty());
}

TEST_CASE("A bus that throws mid-scan yields no rpiboot devices", "[rpiboot][scan]")
{
    // The scan runs on the drive-poll thread. An exception escaping here
    // would take the poll thread with it and the device list would stop
...
// The overload the drive-poll thread actually calls, which makes its own
// libusb context rather than being handed one. If libusb will not start --
// no USB support in the kernel, no permission on the bus, or, as here, no
// descriptor left to give it -- the constructor throws, and an exception
// reaching the poll thread stops the device list updating for the session.
          ...
          "[rpiboot][scan]")
{
    ...
    tight.rlim_cur = 3;  // stdin, stdout, stderr; nothing to spare
    REQUIRE(setrlimit(RLIMIT_NOFILE, &tight) == 0);

    bool contextRefused = false;
    try {
    ...
    bool threw = false;
    try {
        found = rpiboot::scanRpibootDevices();
    } catch (...) {
        threw = true;
    }
    ...
    // Asserted rather than assumed: if libusb ever starts without a spare
    // descriptor this test stops exercising the arm it is named for, and
    // should say so rather than pass on the empty list it would still get.
...
class RealBusRpibootThread : public RpibootThread
{
public:
    using RpibootThread::RpibootThread;
    using RpibootThread::makeUsbContext;
    using RpibootThread::pollForRpibootReturn;
};

// Every phase of the sideload starts by asking for a USB context, and
// LibusbContext's constructor throws when libusb will not initialise. This
// runs inside QThread::run(), where an escaping exception terminates the
// process rather than failing the write -- so it is caught and turned into
// a nullptr the phases check. Descriptor exhaustion is the arm that can be
// arranged without unloading a kernel module.
          ...
          "[rpiboot][usb]")
{
    ...
    std::unique_ptr<rpiboot::IUsbContext> ctx;
    bool threw = false;
    try {
        ctx = t.makeUsbContext();
    } catch (...) {
        threw = true;
    }
...
// device. Without a bus to watch there is nothing to wait for, and the
// wait has to end rather than spend a minute polling a context it does
// not have.
TEST_CASE("Waiting for the board to return ends at once with no bus", "[rpiboot][usb]")
{
    TestableRpibootThread t{chosenDevice(), rpiboot::SideloadMode::SecureBootRecovery};
    t.busUnavailable = true;
    ...
    QElapsedTimer timer;
    timer.start();
    CHECK_FALSE(t.pollForRpibootReturn(found, 9));
    // Returned on the missing context, not after 120 half-second polls.
