 ...
 * not have.
 *
 * So the dialog asks the user to type the device's serial number out. That is
 * a deliberately awkward thing to ask, and every part of the awkwardness is
 * load-bearing: the match is exact, the button stays dead until it matches,
 * pasting is refused so the serial has to be read off the screen rather than
 * copied from it, and reopening starts again from empty.
 *
 * None of it had a test -- the whole file was uncovered.
 */
