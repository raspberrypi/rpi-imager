 ...
 *
 * The list mixes three kinds of row that look alike and must behave
 * differently: a real OS, a category to descend into, and the "Go back"
 * entry a sublist adds at the end. Keyboard selection of a real OS advances
 * to the card chooser straight away, which is the convenience -- and it is
 * why the other two must not. Pressing Enter on "Emulation and game OS"
 * cannot carry someone to the next step: there would be no image chosen,
 * and the row they picked has no URL to write.
 *
 * The existing files cover the list view and how a sublist is detected;
 * neither had ever selected a row through the step's own handlers.
 */
