 ...
 *
 * "No" and escape have to close it and leave the user where they were. A
 * prompt that cannot be dismissed sits in front of the wizard on a machine
 * the user was about to image; one that treats a dismissal as agreement
 * sends them to a download page they did not ask for, losing their place.
 *
 * The Update button opens a browser, which is not something to do on the
 * machine running the suite, so the opening is a named method the test
 * shadows -- the same arrangement as the help links on the option rows and
 * the Connect sign-in button.
 */
