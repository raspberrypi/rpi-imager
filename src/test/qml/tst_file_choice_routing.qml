 ...
 *
 * So setting a custom repository, on the screen where a user is most likely
 * to set one, also went through the custom-image handler: the chosen OS
 * became the repository file, and every customisation the user had staged --
 * Wi-Fi, the user account, SSH -- was cleared along with it, because a
 * custom image cannot carry customisation. Measured before the fix, on a
 * container sitting on the OS step with Wi-Fi, a user account and SSH
 * configured:
 *
 *     BEFORE osName=Raspberry Pi OS (64-bit) wifi=true  user=true  ssh=true
 *     AFTER  osName=my-repo.json             wifi=false user=false ssh=false
 *
 * Nothing tells the user. They go on through the wizard and write a json
 * file to the card, with none of the setup they asked for.
 *
 * A selection now says what it is answering, and each chooser takes only its
 * own. Both directions are checked here: the wrong one being ignored is only
 * half of it, and a guard that rejected everything would leave "use custom"
