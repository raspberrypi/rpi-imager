 ...
 *
 * Every progress signal from the writer arrives at main.qml, is handed to the
 * wizard container, and is forwarded by it to the writing step -- but only
 * while the writing step is the one on screen. Six forwarding functions, none
 * of them covered, and the guard on each is the interesting part: a write runs
 * on its own thread, so a late signal can land after the user has moved on,
 * and forwarding it would call a progress method on a step that has none.
 *
 * The step's own guard is the second one. Progress arriving when no write is
 * running must not move the bar: the write has finished, and a bar that jumps
 * back to 40% on the completion screen says the card is still being written
 * when it is safe to unplug.
 *
 * Both guards are checked here, in both directions, along with the arithmetic
 * behind the number the user reads. A progress signal reporting a total of
 * zero -- which is what a stream of unknown length gives -- must not divide by
 * it: the bar would be handed Infinity and show whatever that renders as.
 *
 * The write state is driven through the writer's own slots, the way
 * tst_writing_step does: onFinalizing() lands in a state that counts as
 * writing, and onCancelled() leaves it.
 */
