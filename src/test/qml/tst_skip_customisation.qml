 ...
 *
 * The handler behind it used to be written out in each of the eight steps
 * that offer the button, in four different versions: five cleared five flags,
 * SecureBoot and PiConnect cleared those plus their own, and IfAndFeatures
 * cleared ten. if1WireEnabled was cleared by none of them, although every
 * other clear-all path in the container includes it.
 *
 * The steps are not in a fixed order relative to each other for this purpose
 * -- the sidebar lets a user go back to any step already marked permissible
 * -- so enabling an interface on the Interfaces screen and then going back to
 * Wi-Fi and pressing Skip is a route a user can take. On that route the
 * writing step's summary, which is the last thing shown before the erase,
 * lists interfaces the user has just declined to configure, while the
 * completion screen -- which reads a snapshot rather than the live flags --
 * lists none. The two screens disagree about the same card.
 *
 * This checks the property rather than any one implementation of it: after
 * Skip on any step, no customisation flag is left set. The eight copies are
 * now one function on the container, which is what makes that true.
 */
