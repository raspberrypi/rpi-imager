 ...
 *
 * The consequence of getting it wrong is not subtle. Leave the navigation
 * buttons off the end and a keyboard user can reach every field on the
 * screen and not the button that leaves it -- the wizard cannot be
 * completed at all without a mouse. Drop the wrap-around and Tab past the
 * last control returns nothing, so focus goes wherever the framework
 * decides, usually nowhere visible.
 *
 * WizardStepBase is instantiated directly here rather than through one of
 * the steps: what is under test is the base's own arithmetic, and a real
 * step brings its own controls, its own capability lookups and its own
 * deferred initialisation along with it.
 *
 * Worth knowing before reading these: the file holds two tab orders that do
 * not agree. rebuildFocusOrder() wires KeyNavigation.tab as Next, Back,
 * Skip, App Options, then the registered controls; these two functions,
 * described in the source as the "global Tab navigation fallback", put the
 * registered controls first and the buttons after. So which order a user
 * gets depends on which mechanism handles the key. That is recorded rather
 * than corrected -- where the buttons belong in the order is a decision
 * about the interface, not a defect to fix from a test. What is pinned here
 * is that the fallback is a complete ring: everything reachable, wrapping
 * at both ends, and nothing in it that is not on the screen.
 */
