...
TestCase {
    ...
    //
    // The destructive one is the point. "Clear" removes the hostname, the
    // wireless network, the user account and its password from disk, for
    // good, and the only thing between a mis-click and that is a
    // confirmation. So both of its answers matter, and so does escape: a
    // key pressed to get rid of a dialog must never be the one that
    // empties the settings behind it.
