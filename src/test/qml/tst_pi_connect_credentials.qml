 ...
 *
 * In organisation mode the step holds an organisation API key, saved between
 * sessions and deliberately never shown again -- so the only evidence a user
 * has that it is stored is a placeholder and a "Clear saved key" button. That
 * button is therefore the whole of the removal story: if it clears the fields
 * but not the setting, the user is told the key is gone while it is still on
 * disk and still being sent with the next write. Nothing covered it.
 *
 * In the ordinary flow the credential is a per-user authentication token, and
 * the field it goes in is locked until either the browser delivers one or a
 * 25-second countdown runs out. Both halves matter: a token delivered by the
 * browser must leave the field locked, so it cannot be edited into something
 * that will not enrol, and a browser callback that never arrives -- no URI
 * handler registered, which is easy to end up with on Linux -- has to leave
 * the user a way to paste the token by hand, or the step is a dead end.
 *
 * The countdown itself is started only by the "Open Raspberry Pi Connect"
 * button, whose own handler shells out through xdg-open, so the timer's tick
 * is not driven from here. What is checked is the state it moves between and
