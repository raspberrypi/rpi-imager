 ...
 *
 * The clamp is Math.max(min, Math.min(max, width)), which passes anything that
 * is not a number straight through as NaN. The width then lands in an int
 * property, where NaN becomes 0. A sidebar of no width takes the whole
 * navigation off the screen, and the value that produced it is a persisted
 * setting, so it happens again on every launch.
 *
 * Reaching it needs a sidebarWidth setting that is not a number, which the
 * application never writes; a corrupted or hand-edited settings file does. It
 * is recoverable -- the drag handle has its own width in the layout, so it
 * survives a sidebar of none, and double-clicking it restores the default --
 * but only for a user who thinks to double-click a one-pixel separator. The
 * cost is out of proportion to the odds, so the clamp now answers with the
 * default rather than with something that cannot be a width.
 */
