 ...
 *
 * They are worth reaching for what they are: this is the click that decides
 * which disk gets erased. A click that selects the wrong row, or a read-only
 * row that responds to one, is not a cosmetic fault.
 *
 * The rows are real. TestDrives hands a device list to the same slot the
 * drive poller calls, so the model underneath is the DriveListModel the step
 * binds to, filled through its own insertion path with its own roles. A
 * look-alike model would answer whatever the delegate asked of it, including
 * after somebody renamed a role.
 *
 * Worth knowing which rows carry weight. Dropping the selection from the
 * delegate's click fails three cases; making the double click do nothing
 * fails one. The read-only refusals are another matter: the same "is this row
 * selectable" question is asked three times over -- the MouseArea is
 * disabled, the click handler checks, and selectDstItem() checks again -- so
 * no single one of them can be removed and noticed here. Those rows hold the
 * outcome rather than any one guard, and only removing the lot fails them.
 * That is still worth having, because the outcome is what the user meets, but
 * a change that quietly drops one of the three will pass.
 */
