...
TEST_CASE("A machine with no sound theme still gets a chime", "[platformquirks][beep]")
{
    // beep() prefers a system sound file and works down a list of distro
    // locations. When none is readable it extracts a bundled chime from Qt
    // resources instead -- which is what a minimal image, or an embedded
    // Pi with no sound theme installed, actually has. That fallback had
    // never run: every machine the suite runs on has one of the five.
    if (!haveMountNamespaces())
        SKIP("unprivileged mount namespaces are unavailable, so /usr/share/sounds "
             "cannot be replaced");

    QTemporaryDir emptySounds;
    REQUIRE(emptySounds.isValid());

    QProcess p;
    p.start(QStringLiteral("unshare"),
            {QStringLiteral("-rm"), QStringLiteral("--propagation"),
             QStringLiteral("private"), QStringLiteral("sh"), QStringLiteral("-c"),
             QStringLiteral("mount --bind \"$1\" /usr/share/sounds && exec \"$2\" soundfile"),
             QStringLiteral("_"), emptySounds.path(),
             QStringLiteral(BEEP_PROBE_BINARY)});
    REQUIRE(p.waitForFinished(90000));

    const QString out = QString::fromUtf8(p.readAllStandardOutput());
    const QString err = QString::fromUtf8(p.readAllStandardError());
    INFO("stdout: " << out.toStdString());
    INFO("stderr: " << err.toStdString());

    CHECK(out.contains(QStringLiteral("DONE=1")));
    // It said which chime it fell back to, and it was the bundled one.
    CHECK(err.contains(QStringLiteral("Using bundled fallback chime")));
}

#endif // BEEP_PROBE_BINARY

TEST_CASE("Network monitoring that cannot get a descriptor cleans up after itself",
          "[platformquirks][netdetect]")
{
    // startNetworkMonitoring() opens a netlink socket and then an eventfd to
    // signal the thread to stop. Running out of descriptors between the two
    // is the ordinary way this fails on a busy desktop, and the arm that
    // handles it -- closing the socket again rather than leaking it and
    // returning rather than starting a thread with no way to stop it -- had
    // never run.
    //
    // The probe lowers RLIMIT_NOFILE until exactly one descriptor is free,
    // so the socket is opened and the eventfd is not. No namespace needed.
    QProcess p;
    p.start(QStringLiteral(NETWORK_PROBE_BINARY), {QStringLiteral("nofd")});
    REQUIRE(p.waitForFinished(30000));
    ...
    if (out.contains(QStringLiteral("NOFD=skip")))
        SKIP("the descriptor limit could not be tightened on this machine");

    // Returned rather than aborted, and said why on the way past.
    CHECK(out.contains(QStringLiteral("NOFD=survived")));
    CHECK(p.exitStatus() == QProcess::NormalExit);
    CHECK(err.contains(QStringLiteral("stop eventfd")));
    // And it did not go on to claim monitoring had started.
    CHECK_FALSE(err.contains(QStringLiteral("Network monitoring started")));
}

TEST_CASE("Network monitoring that cannot start its thread cleans up after itself",
          "[platformquirks][netdetect]")
{
    // The socket and the stop eventfd are both open by the time the thread
    // is created, so the failure arm has two descriptors to close and a
    // running flag to clear. Leaking either costs a descriptor per attempt;
    // leaving the flag set makes a later stop hang waiting for a thread that
    // was never started.
    QProcess p;
    p.start(QStringLiteral(NETWORK_PROBE_BINARY), {QStringLiteral("nothread")});
    REQUIRE(p.waitForFinished(30000));
    ...
    if (out.contains(QStringLiteral("NOTHREAD=skip")))
        SKIP("the thread limit could not be lowered on this machine");

    CHECK(out.contains(QStringLiteral("NOTHREAD=survived")));
    CHECK(p.exitStatus() == QProcess::NormalExit);
    CHECK(err.contains(QStringLiteral("Failed to create monitor thread")));
    // And it did not claim to have started.
    CHECK_FALSE(err.contains(QStringLiteral("Network monitoring started")));
}

TEST_CASE("Opening a URL when the process cannot fork is reported, not assumed",
          "[platformquirks][url]")
{
    // With no desktop portal, openUrlExternally() falls back to running
    // xdg-open. If the fork is refused it must say so and return false --
    // returning true would leave the caller believing a browser opened, and
    // the caller's fallback (QDesktopServices) never runs.
    QProcess p;
    p.start(QStringLiteral(NETWORK_PROBE_BINARY), {QStringLiteral("nofork")});
    REQUIRE(p.waitForFinished(30000));
    ...
    if (out.contains(QStringLiteral("NOFORK=skip")))
        SKIP("the process limit could not be lowered on this machine");

    CHECK(out.contains(QStringLiteral("NOFORK=0")));
    CHECK(err.contains(QStringLiteral("fork failed")));
}

TEST_CASE("A URL is offered to the desktop portal before xdg-open",
          "[platformquirks][url]")
{
    // openUrlExternally() prefers the xdg-desktop-portal OpenURI interface,
    // which is the only path that works from inside a confined Flatpak or
    // Snap. None of it had run: the suite has no session bus, and without
    // one the function returns before building the call.
    //
    // dbus-run-session gives it a bus of its own, so nothing touches the
    // session the suite runs in. Whether a portal answers there is not the
    // point -- what is pinned is that the attempt is made and a refusal
    // falls through to xdg-open rather than reporting failure.
    {
        QProcess ver;
        ver.start(QStringLiteral("dbus-run-session"), {QStringLiteral("--version")});
        if (!ver.waitForFinished(10000) || ver.exitCode() != 0)
            SKIP("dbus-run-session is not installed, so no private bus can be made");
    }

    QProcess p;
    p.start(QStringLiteral("dbus-run-session"),
            {QStringLiteral("--"), QStringLiteral(NETWORK_PROBE_BINARY),
    ...
    // The attempt was made: either the portal answered, or it said why not.
    CHECK((err.contains(QStringLiteral("OpenURI portal unavailable"))
           || err.contains(QStringLiteral("Opened URL via xdg-desktop-portal"))));
    // And the caller was told the URL was handled, one way or the other.
    CHECK(out.contains(QStringLiteral("PORTAL=1")));
}
...
TEST_CASE("A policy that cannot be written is removed, not left truncated",
          "[platformquirks][policyinstall]")
{
    // The policy file is created and then written to. A short write leaves
    // a truncated XML document in polkit's actions directory, and polkit
    // refuses to parse one -- so every later elevation fails, silently,
    // because of a file we put there. The arm that unlinks it again had
    // never run.
    REQUIRE_POLICY_HARNESS();
    ...
    const QString out = runProbeWithNoActionsDir(QStringLiteral("installtiny"),
                                                 etc.path(), usr.path());
    INFO("probe said: " << out.toStdString());

    if (out.contains(QStringLiteral("TINY=skip")))
        SKIP("the file-size limit could not be lowered on this machine");

    // Reported as failed rather than claimed.
    CHECK(out.contains(QStringLiteral("INSTALLED=0")));

    // And nothing was left behind: a truncated .policy here is worse than
    // none, because polkit stops reading the directory at the broken file.
    ...
    const QStringList written = actions.entryList(QStringList()
                                                  ...
                                                  QDir::Files);
    INFO("left behind: " << written.join(QStringLiteral(", ")).toStdString());
...
// ══════════════════════════════════════════════════════════════
// Taking a card's partitions offline before the write opens it
//
// unmountDisk() calls umount(2) in the calling process, so the two cases
// above it skip unless the whole suite runs as root -- which it does not,
// and should not. The result is that the entire unmount cascade shipped
// untested: a card whose partition is still mounted when the write opens
// the device is how a half-written filesystem gets flushed over the image
// that replaced it.

namespace {

struct UnmountProbeResult
{
    bool ran = false;
    QString out;
...
UnmountProbeResult runUnmountProbe(const QString& mode)
{
    UnmountProbeResult r;
    QProcess p;
    p.start(QStringLiteral("unshare"),
            {QStringLiteral("-rm"), QStringLiteral("--propagation"),
             QStringLiteral("private"), QStringLiteral(UNMOUNT_PROBE_BINARY), mode});
    ...
    r.ran = true;
    // Both streams: the values are on stdout, the cascade's own account of
    // which attempt worked is on stderr, and a failure wants to show it.
    r.out = QString::fromUtf8(p.readAllStandardOutput())
...
TEST_CASE("A mounted partition is taken offline before the write", "[platformquirks][disk]")
{
    if (!haveMountNamespaces())
        SKIP("unprivileged mount namespaces are unavailable");

    const UnmountProbeResult r = runUnmountProbe(QStringLiteral("clean"));
    if (!r.ran)
        SKIP("the unmount probe did not run");
    INFO("probe said:\n" << r.out.toStdString());

    // It really was mounted, and really is not afterwards. Without the
    // first of those the case passes on a fixture that never worked.
    CHECK(r.value(QStringLiteral("MOUNTS_BEFORE")) == 1);
    CHECK(r.value(QStringLiteral("SUCCESS")) == 1);
    CHECK(r.value(QStringLiteral("MOUNTS_AFTER")) == 0);
}

TEST_CASE("A partition with a file still open on it is taken offline anyway",
          "[platformquirks][disk]")
{
    // The ordinary case on a desktop: a file manager indexing the card, or
    // a shell sitting in a directory on it, so the plain umount and both
    // MNT_EXPIRE attempts come back EBUSY. The cascade drops to a lazy
    // unmount rather than giving up -- giving up would mean writing to a
    // device the kernel still has a filesystem mounted on.
    if (!haveMountNamespaces())
        SKIP("unprivileged mount namespaces are unavailable");

    const UnmountProbeResult r = runUnmountProbe(QStringLiteral("busy"));
    if (!r.ran)
        SKIP("the unmount probe did not run");
    INFO("probe said:\n" << r.out.toStdString());
    ...
    CHECK(r.value(QStringLiteral("SUCCESS")) == 1);
    CHECK(r.value(QStringLiteral("MOUNTS_AFTER")) == 0);
    // The file really was open, which is what makes this the cascade and
    // not the first attempt succeeding: a plain umount answers EBUSY here.
    CHECK(r.value(QStringLiteral("HELD_OPEN")) == 1);
}

TEST_CASE("A card with nothing mounted is not an unmount failure",
          "[platformquirks][disk]")
{
    // A blank card. Reporting an error here would block writing to a
    // perfectly good one.
    if (!haveMountNamespaces())
        SKIP("unprivileged mount namespaces are unavailable");

    const UnmountProbeResult r = runUnmountProbe(QStringLiteral("none"));
    if (!r.ran)
        SKIP("the unmount probe did not run");
    INFO("probe said:\n" << r.out.toStdString());

    CHECK(r.value(QStringLiteral("MOUNTS_BEFORE")) == 0);
    CHECK(r.value(QStringLiteral("SUCCESS")) == 1);
}

TEST_CASE("A device that is not there, or is a directory, is refused",
          "[platformquirks][disk]")
{
    // Both are caller mistakes rather than card conditions, and both must
    // be told apart from "nothing to unmount" -- which reports success and
    ...
    const int invalid = static_cast<int>(PlatformQuirks::DiskResult::InvalidDrive);
    CHECK(r.value(QStringLiteral("MISSING_RESULT")) == invalid);
    CHECK(r.value(QStringLiteral("DIRECTORY_RESULT")) == invalid);
    // And not the same answer as the blank card above.
    CHECK(r.value(QStringLiteral("RESULT")) != invalid);
}
#endif  // UNMOUNT_PROBE_BINARY

#ifdef ELEVATION_PROBE_BINARY
TEST_CASE("Stale policies are cleared out, and the ones in use are left alone",
          "[platformquirks][policyinstall]")
{
    // An AppImage that has been moved, renamed or deleted leaves its polkit
    // policy behind pointing at a path with nothing at it. They accumulate
    // one per location the user ever ran it from, and each is an entry
    // granting pkexec rights to a filename anyone could later create. The
    // install sweeps them, and the sweep had never run.
    REQUIRE_POLICY_HARNESS();

    QTemporaryDir etc, usrActions;
    REQUIRE(etc.isValid());
    REQUIRE(usrActions.isValid());

    const QString self = QStringLiteral(ELEVATION_PROBE_BINARY);
    const QString tag = QStringLiteral("org.freedesktop.policykit.exec.path\">");

    const auto policy = [&](const QString& name, const QString& body) {
        QFile f(usrActions.filePath(name));
        REQUIRE(f.open(QIODevice::WriteOnly));
        const QByteArray xml = (QStringLiteral("<policyconfig>") + body
                                + QStringLiteral("</policyconfig>\n")).toUtf8();
        REQUIRE(f.write(xml) == xml.size());
    };
    ...
    // And one that is not ours: the sweep matches on our own naming, and
    // deleting somebody else's policy would take away rights we never gave.
    policy(QStringLiteral("org.example.something-else.policy"),
           QStringLiteral("<annotate key=\"") + tag
               + QStringLiteral("/nonexistent/other</annotate>"));

    const QString out = runProbeInPolkitNamespace(QStringLiteral("install"),
                                                  etc.path(), usrActions.path());
    INFO("probe said: " << out.toStdString());
    REQUIRE_FALSE(out.isEmpty());

    const QDir dir(usrActions.path());
...
#ifdef ELEVATION_PROBE_BINARY
TEST_CASE("An AppImage in a deeply nested directory still gets its policy",
          "[platformquirks][policyinstall]")
{
    // The policy filename is built from the AppImage's own path into a
    // fixed 256-byte buffer, and an AppImage lives wherever the user put
    // it -- a deep Nextcloud or Downloads tree is ordinary. Written
    // expecting the install to refuse a path that long; it does not, which
    // is the better answer and worth pinning. What would be wrong is a
    // truncated name: two installs under different long paths sharing one
    // policy, or polkit granting rights under a path that is not the one
    // asked about.
    REQUIRE_POLICY_HARNESS();
    ...
    // Long enough that the mangled path cannot fit, without approaching
    // PATH_MAX: each segment is short, there are simply many of them.
    QString nested = deep.path();
    for (int i = 0; i < 24; ++i)
        nested += QStringLiteral("/anested-directory-segment");
    REQUIRE(QDir().mkpath(nested));

    const QString copied = nested + QStringLiteral("/rpi-imager.AppImage");
    REQUIRE(QFile::copy(QStringLiteral(ELEVATION_PROBE_BINARY), copied));
    ...
    p.start(QStringLiteral("unshare"),
            {QStringLiteral("-rm"), QStringLiteral("--propagation"),
             ...
             QStringLiteral("mount --bind \"$1\" /etc/polkit-1 "
                            "&& mount --bind \"$2\" /usr/share/polkit-1/actions "
                            "&& exec \"$3\" install"),
             QStringLiteral("_"), etc.path(), usrActions.path(), copied});
    if (!p.waitForFinished(30000)) {
        p.kill();
        p.waitForFinished(5000);
        SKIP("the namespace could not be built");
    ...
    // It saw the long path and installed anyway.
    CHECK(out.contains(QStringLiteral("INSTALLED=1")));
    CHECK(out.contains(QStringLiteral("anested-directory-segment")));

    const QStringList written = QDir(usrActions.path()).entryList(
        QStringList() << QStringLiteral("*.policy"), QDir::Files);
    INFO("written: " << written.join(QStringLiteral(", ")).toStdString());
    REQUIRE(written.size() == 1);

    // One file, named within the buffer, and carrying the whole path it
    // grants rights to rather than a cut-down version of it.
    CHECK(written.first().size() < 256);
    QFile installed(QDir(usrActions.path()).filePath(written.first()));
    REQUIRE(installed.open(QIODevice::ReadOnly));
    const QByteArray body = installed.readAll();
    CHECK(body.contains(copied.toUtf8()));
}

TEST_CASE("With nothing installed, the elevation policy reads as absent",
          "[platformquirks][policyinstall]")
{
    // What decides whether Imager offers to install one. Answering yes on
    // a machine with no policy means the offer never appears and every
    // write that needs elevation fails with a permissions error instead of
    // an explanation; answering yes wrongly is the worse direction.
    REQUIRE_POLICY_HARNESS();
    ...
    // A policy directory with a policy in it -- for something else.
    {
        QFile f(usrActions.filePath(QStringLiteral("org.example.other.policy")));
        REQUIRE(f.open(QIODevice::WriteOnly));
        f.write("<policyconfig></policyconfig>\n");
    }

    const QString out = runProbeInPolkitNamespace(QStringLiteral("policy"),
                                                  etc.path(), usrActions.path());
    INFO("probe said: " << out.toStdString());
    REQUIRE_FALSE(out.isEmpty());

    // Read the directory, found nothing of ours, and said so.
    CHECK(out.contains(QStringLiteral("POLICY=0")));
}
#endif  // ELEVATION_PROBE_BINARY

#ifdef BEEP_PROBE_BINARY
TEST_CASE("The chime falls through to whichever player the machine has",
          "[platformquirks][beep]")
{
    // beep() tries four players in order -- paplay, pw-play, aplay, pactl --
    // and stops at the first that works. Only the head of that chain had
    // ever run, so on a machine with ALSA but no PipeWire, or an older one
    // with only PulseAudio, the arm that actually makes the sound was
    // untested. A chime that never plays is how somebody who looked away
    // misses that their card is finished.
    if (!haveASystemSoundFile())
        SKIP("no system sound file on this host, so beep() has nothing to play");

    SECTION("ALSA only") {
        QTemporaryDir bin, marks;
        REQUIRE(bin.isValid());
        REQUIRE(marks.isValid());
        // Nothing earlier in the chain exists, so aplay is reached; it
        // succeeds, so pactl is never tried.
        REQUIRE(plantAudioTool(bin.path(), QStringLiteral("aplay"), marks.path(), 0));
        REQUIRE(plantAudioTool(bin.path(), QStringLiteral("pactl"), marks.path(), 0));

        const QString out = runBeepProbe(bin.path(), QString());
        INFO("probe said: " << out.toStdString());
        CHECK(out.contains(QStringLiteral("DONE=1")));

        CHECK(QFileInfo::exists(marks.filePath(QStringLiteral("aplay"))));
        // Stopped there: playing the chime twice is worse than once.
        CHECK_FALSE(QFileInfo::exists(marks.filePath(QStringLiteral("pactl"))));
    }

    SECTION("PulseAudio only") {
        // The last resort, and the only one that takes two commands: the
        // sample is uploaded and then played, so a fixture that answers the
        // upload but not the play would leave the machine silent.
        QTemporaryDir bin, marks;
        REQUIRE(bin.isValid());
        REQUIRE(marks.isValid());
        REQUIRE(plantAudioTool(bin.path(), QStringLiteral("pactl"), marks.path(), 0));
        ...
        INFO("probe said: " << out.toStdString());
        CHECK(out.contains(QStringLiteral("DONE=1")));
        CHECK(QFileInfo::exists(marks.filePath(QStringLiteral("pactl"))));
    }

    SECTION("a player that is there but fails") {
        // Present and refusing -- no sound card, or a busy device. The chain
        // has to carry on to the next rather than stop at the failure.
        QTemporaryDir bin, marks;
        REQUIRE(bin.isValid());
        REQUIRE(marks.isValid());
        REQUIRE(plantAudioTool(bin.path(), QStringLiteral("aplay"), marks.path(), 1));
        REQUIRE(plantAudioTool(bin.path(), QStringLiteral("pactl"), marks.path(), 0));
