 ...
 *
 * The revision code is a hex word in /proc/cpuinfo whose bits 4-11 name the
 * model. The path is hardcoded, so these run a probe under `unshare -rm`
 * with a /proc/cpuinfo of their own bind-mounted over the real one -- which
 * is also what lets them ask about hardware that does not exist yet.
 */
