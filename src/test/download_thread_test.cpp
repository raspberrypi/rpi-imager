...
// The async queue holds one write block per slot, so the depth decides how
// much RAM the writer reserves. Two caps guard it: a hard 256, and a refusal
// to reserve more than half of what is available. On any machine a test runs
// on there is plenty of RAM, so the memory cap never fired and went unrun --
// which matters most on the machines it exists for, a 512 MB Pi Zero.
TEST_CASE("An async queue depth is capped by memory as well as by count", "[download][memory]")
{
    constexpr size_t kBlock = 4u * 1024 * 1024;  // 4 MB write blocks

    SECTION("a sensible request on a large machine is left alone")
    {
        CHECK(DownloadThread::safeAsyncQueueDepth(16, 8000, kBlock) == 16);
    }

    SECTION("the hard cap applies however much memory there is")
    {
        CHECK(DownloadThread::safeAsyncQueueDepth(100000, 64000, kBlock) == 256);
    }

    SECTION("a request below one slot is raised to one")
    {
        CHECK(DownloadThread::safeAsyncQueueDepth(0, 8000, kBlock) == 1);
        CHECK(DownloadThread::safeAsyncQueueDepth(-5, 8000, kBlock) == 1);
    }

    SECTION("a small machine has its depth cut to fit half its memory")
    {
        // 512 MB available, 4 MB blocks: half is 256 MB, so 64 slots.
        CHECK(DownloadThread::safeAsyncQueueDepth(256, 512, kBlock) == 64);
        // And asking for less than the cap still gets what it asked for.
        CHECK(DownloadThread::safeAsyncQueueDepth(8, 512, kBlock) == 8);
    }

    SECTION("a very small machine still gets a usable queue")
    {
        // Below four slots the depth is floored rather than taken to zero:
        // a queue that holds nothing wedges the writer instead of slowing it.
        // Asserted exactly -- ">= 4" was also satisfied by the uncapped 256,
        // so it passed with the cap removed.
        CHECK(DownloadThread::safeAsyncQueueDepth(256, 8, kBlock) == 4);
    }

    SECTION("nonsense inputs do not divide by zero or go negative")
    {
        CHECK(DownloadThread::safeAsyncQueueDepth(16, 8000, 0) == 16);
        CHECK(DownloadThread::safeAsyncQueueDepth(16, 0, kBlock) == 16);
        CHECK(DownloadThread::safeAsyncQueueDepth(16, -1, kBlock) == 16);
    }
}

// ══════════════════════════════════════════════════════════════
// Downloading into memory rather than onto a device
//
// With no destination filename DownloadThread keeps what it fetched in a
// buffer, and data(), lastModified() and serverTime() are how a caller gets
// at it. Reaching it needs isImage() to answer false, and no subclass in the
// tree does: the application's listings are fetched elsewhere, so this whole
// mode is public API with no caller today. It is tested rather than assumed
// because that is the choice in front of whoever reads it next -- it either
// behaves as its signature says, or it should go.
// ══════════════════════════════════════════════════════════════

// A DownloadThread in the mode its buffer branch exists for.
class InMemoryDownload : public DownloadThread
{
public:
    using DownloadThread::DownloadThread;
    bool isImage() override { return false; }
};

TEST_CASE("A download with no destination is kept in memory", "[download][memory]")
{
    QTemporaryDir served;
    REQUIRE(served.isValid());
    const QByteArray body = "{\"os_list\": []}";
    {
        QFile f(served.filePath(QStringLiteral("os_list.json")));
        REQUIRE(f.open(QIODevice::WriteOnly));
        f.write(body);
    }

    LocalHttpServer server(served.path());
    REQUIRE_HTTP_SERVER(server);

    InMemoryDownload dt(server.urlFor(QStringLiteral("os_list.json")), QByteArray(), QByteArray());
    dt.start();
    REQUIRE(dt.wait(30000));

    CHECK(dt.successfull());
    CHECK(dt.data() == body);

    // Both come from the response headers and are what decides whether a
    // cached listing is still good. A server that sends neither leaves them
    // at zero rather than at something invented.
    CHECK(dt.lastModified() >= 0);
    CHECK(dt.serverTime() >= 0);
}

TEST_CASE("A cancelled download keeps nothing", "[download][memory]")
{
    // Cancelling makes the write callback return zero, which is how curl is
    // stopped from the outside. Nothing may be appended after that point:
    // half a listing parses as a listing.
    QTemporaryDir served;
    REQUIRE(served.isValid());
    {
        QFile f(served.filePath(QStringLiteral("os_list.json")));
        REQUIRE(f.open(QIODevice::WriteOnly));
        f.write(QByteArray(4 * 1024 * 1024, 'x'));
    }
...
// The caching side of the same callback. Caching is asked for per download
// and turned off the moment the writer behind it is unusable, because the
// alternative is stalling the curl callback -- and a download that stops to
// wait for a cache write is worse than one with no cache at all.
class CacheStateThread : public DownloadThread
{
public:
    using DownloadThread::DownloadThread;
    using DownloadThread::_writeCache;
    using DownloadThread::_writeData;
    using DownloadThread::_fileGetContentsTrimmed;
    using DownloadThread::_cacheEnabled;
    using DownloadThread::_asyncCacheWriter;
};

TEST_CASE("Caching turns itself off rather than stalling the download",
          "[download][cache]")
{
    CacheStateThread dt(QByteArray("http://example.invalid/x.img"), QByteArray(), QByteArray());

    SECTION("with no writer behind it") {
        // Asked for, but never started -- the cache directory was not
        // usable. The download still has to run.
        dt._cacheEnabled = true;
        REQUIRE(dt._asyncCacheWriter == nullptr);

        const char payload[] = "some bytes";
        CHECK_NOTHROW(dt._writeCache(payload, sizeof(payload)));
        CHECK_FALSE(dt._cacheEnabled);
    }

    SECTION("and it stays off once it is off") {
        dt._cacheEnabled = false;
        const char payload[] = "some bytes";
        CHECK_NOTHROW(dt._writeCache(payload, sizeof(payload)));
        CHECK_FALSE(dt._cacheEnabled);
    }
...
TEST_CASE("A file read for a device's card identifiers is trimmed, and tolerates absence",
          "[download]")
{
    // sysfs exposes an SD card's CID and CSD on some readers, and they are
    // read straight into a debug line. sysfs values carry a trailing
    // newline, which would otherwise be logged as part of the value; a
    // reader that exposes neither must not produce an error.
    QTemporaryDir dir;
    REQUIRE(dir.isValid());
    const QString path = dir.filePath(QStringLiteral("cid"));
    {
        QFile f(path);
        REQUIRE(f.open(QIODevice::WriteOnly));
        f.write("  035344534433320\n");
    }

    CacheStateThread dt{QByteArray(), QByteArray(), QByteArray()};
    CHECK(dt._fileGetContentsTrimmed(path) == QByteArray("035344534433320"));
    CHECK(dt._fileGetContentsTrimmed(dir.filePath(QStringLiteral("csd"))).isEmpty());
}

TEST_CASE("A bottleneck the interface has no words for says nothing", "[download]")
{
    // The string is put in front of the user beside the progress bar. Every
    // state the enum defines has a translation; anything else has to come
    // back empty so the label is blank, rather than showing a number or the
    // previous state's text.
    CHECK_FALSE(DownloadThread::bottleneckStatusText(
        DownloadThread::BottleneckState::Storage).isEmpty());
    CHECK_FALSE(DownloadThread::bottleneckStatusText(
        DownloadThread::BottleneckState::Network).isEmpty());
    CHECK_FALSE(DownloadThread::bottleneckStatusText(
        DownloadThread::BottleneckState::Verifying).isEmpty());
    // None is a defined case and answers empty; so does a value the enum
    // does not define, which is the arm that stops a future state showing
    // the previous one's words until someone adds a case for it.
    CHECK(DownloadThread::bottleneckStatusText(
        DownloadThread::BottleneckState::None).isEmpty());
    CHECK(DownloadThread::bottleneckStatusText(
        static_cast<DownloadThread::BottleneckState>(99)).isEmpty());
}

TEST_CASE("Caching stops when the writer behind it has failed", "[download][cache]")
{
    // The writer runs on its own thread, so a failure there surfaces here
    // rather than at the call that caused it. It must not be waited on:
    // cancel() blocks for seconds while the thread winds down, and this runs
    ...
    // thread. Either way caching is off and the download is unaffected.
    CHECK_FALSE(opened);

    dt._cacheEnabled = true;
    const char payload[] = "some bytes";
    CHECK_NOTHROW(dt._writeCache(payload, sizeof(payload)));
    // Nothing queued to a writer that never opened, and no stall waiting to
    // find that out.
    CHECK_FALSE(dt._asyncCacheWriter->isActive());
}

TEST_CASE("A write with no device open has no async writes outstanding",
          "[download]")
{
    // The watchdog asks this on a timer while a write is running, and the
    // answer decides whether it hot-swaps to sync mode or restarts. Asked
    // before any device is open -- which is where the watchdog starts --
    // it must answer none rather than reach through a null handle.
    CacheStateThread dt{QByteArray("http://example.invalid/x.img"), QByteArray(), QByteArray()};
    CHECK(dt.pendingAsyncWrites() == 0);
    // And a drain with nothing to drain succeeds rather than waiting out
    // its timeout.
    CHECK(dt.drainAndSwitchToSync(1));
}

TEST_CASE("A UNC path is corrected before curl is given it", "[download]")
{
    // libcurl will not take file://server/share -- it reads the host as a
    // hostname and finds nothing. Windows users pointing at a network share
    // get exactly that form out of the file dialog, so the third slash is
    // added before the handle is configured. The download then fails for the
    // ordinary reason (there is no such share here), which is not what this
    // is about: it is about the URL being rewritten rather than passed on
    // as given.
    QTemporaryDir dir;
    REQUIRE(dir.isValid());
    const QString target = dir.filePath(QStringLiteral("target.img"));
    {
        QFile f(target);
        REQUIRE(f.open(QIODevice::WriteOnly));
        REQUIRE(f.write(QByteArray(1024 * 1024, '\0')) == 1024 * 1024);
    }

    DownloadThread dt(QByteArray("file://rpi-imager-no-such-host/share/os.img"),
                      target.toUtf8(), QByteArray());
    dt.setVerifyEnabled(false);

    QString reported;
    QObject::connect(&dt, &DownloadThread::error,
                     [&reported](QString m) { reported = m; });

    dt.start();
    REQUIRE(dt.wait(60000));
