...
TEST_CASE("A local extraction can be cancelled before it starts", "[extract][local]")
{
    // cancelDownload() now reaches LocalFileExtractThread::_cancelExtract().
    // It did not before: the base declared the method without virtual, so
    // the call bound to the base and the subclass's override never ran at
    // all -- including for the whole of this test, which asserted that it
    // did. Coverage was the only thing that noticed.
    // Observed rather than assumed. A subclass override is only reached
    // through cancelDownload() if the chain is virtual the whole way down;
    // with the base's _cancelExtract() non-virtual the call binds there and
    // this counter stays at zero, which is what it did.
    struct Recording : LocalFileExtractThread {
        using LocalFileExtractThread::LocalFileExtractThread;
        int cancels = 0;
        void _cancelExtract() override { ++cancels; LocalFileExtractThread::_cancelExtract(); }
    };

    Recording dt("file:///nonexistent", "", "");
...
TEST_CASE("A multi-file archive that verifies is kept for next time",
          "[extract][multifile][cache]")
{
    // The cache is what makes writing the same image to a second card fast:
    // the compressed download is kept, and the next write extracts it from
    // disk instead of fetching it again. It is only kept when the hash
    // matches what was expected, because a cache entry that does not match
    ...
    REQUIRE(archiveFile.open(QIODevice::ReadOnly));
    QCryptographicHash h(QCryptographicHash::Sha256);
    REQUIRE(h.addData(&archiveFile));
    const QByteArray expected = h.result().toHex();
    archiveFile.close();
    ...
    dt.setVerifyEnabled(false);
    dt.enableMultipleFileExtraction();
    dt.setCacheFile(cachePath, QFileInfo(archive).size());

    QByteArray reportedImageHash;
    QByteArray reportedCacheHash;
    QObject::connect(&dt, &DownloadThread::cacheFileHashUpdated,
                     [&](QByteArray cacheHash, QByteArray imageHash) {
                         reportedCacheHash = cacheHash;
                         reportedImageHash = imageHash;
                     });
    int legacySignals = 0;
    QObject::connect(&dt, &DownloadThread::cacheFileUpdated,
                     [&legacySignals](QByteArray) { ++legacySignals; });
    ...
    CHECK(reportedImageHash == expected);
    CHECK_FALSE(reportedCacheHash.isEmpty());
    // The older single-hash signal is still emitted: the settings code has
    // not moved off it.
    CHECK(legacySignals == 1);

    // And there is something on disk to reuse.
    CHECK(QFileInfo(cachePath).size() > 0);
}

TEST_CASE("A multi-file archive whose hash is wrong leaves no cache entry",
          "[extract][multifile][cache]")
{
    // The other direction, and the one that matters: a download that does
    // not match must not be left behind as a cache entry, or every later
    ...
    QObject::connect(&dt, &DownloadThread::cacheFileHashUpdated,
                     [&announced](QByteArray, QByteArray) { announced = true; });
...
// ══════════════════════════════════════════════════════════════
// A download that goes quiet part way through
//
// The ring buffers call themselves stalled after ninety seconds without
// progress, and that arm is what turns a write that has silently stopped
// into a message naming the reason. Nothing had reached it: ninety seconds
// of real waiting per stall type is more than a suite can spend, so the
// whole path -- the stall event in the performance report, the error the
// user reads, and libarchive being told to stop rather than left waiting --
// shipped untested.

namespace {

class ImpatientExtract : public DownloadExtractThread
{
public:
    ImpatientExtract(const QByteArray &url, const QByteArray &dst, uint32_t stallMs)
        : DownloadExtractThread(url, dst, QByteArray(), nullptr, stallMs)
    {
        // Through the constructor, not assigned afterwards: the ring buffers
        // are built by the base constructor, so a later assignment arrives
        // after they already have the shipped timeout.
    }
};

} // namespace

TEST_CASE("A download that stops sending is reported as stalled, not left waiting",
          "[extract][stall]")
{
    // Enough of a real archive to get libarchive going, and then silence.
    // A server that sends nothing at all does not reach the stall: nothing
    // is waiting on the buffer yet, and the download layer treats the quiet
    // socket as a lost connection and reconnects once a minute instead.
    if (!haveTool(QStringLiteral("xz")))
        SKIP("xz is not installed, so no compressed fixture can be built");

    ScratchDir scratch;
    const QString raw = scratch.filePath(QStringLiteral("os.img"));
    REQUIRE(writeFile(raw, imageOfSize(4 * 1024 * 1024, 11)));
    REQUIRE(runTool(QStringLiteral("xz"), {QStringLiteral("-T1"), QStringLiteral("-2"), raw}));
    const QString archive = raw + QStringLiteral(".xz");
    REQUIRE(QFileInfo::exists(archive));

    // Enough to get libarchive going, then silence.
    rpi_test::PartialThenSilentHttpServer server(archive, 8192, 64 * 1024 * 1024);
    REQUIRE_HTTP_SERVER(server);

    const QString target = scratch.filePath(QStringLiteral("target.img"));
    REQUIRE(writeFile(target, QByteArray(8 * 1024 * 1024, '\0')));

    // 300 ms, not the shipped ninety seconds: each failed acquire costs
    // about 100 ms, so this is three of them. It has to beat curl, which
    // gives up on a trickle of its own accord and hands the transfer to the
    // resume path -- the buffer's timeout is the backstop behind that, and
    // only gets its turn if it fires first.
    ImpatientExtract dt(server.urlFor(QStringLiteral("os.img.xz")),
                        target.toUtf8(), 300);
    dt.setVerifyEnabled(false);

    QStringList bufferEvents;
    QObject::connect(&dt, &DownloadExtractThread::eventRingBufferStats, &dt,
                     [&bufferEvents](qint64, quint32, QString metadata) {
                         bufferEvents << metadata;
                     });

    const Outcome outcome = runToCompletion(dt, 120000);
    INFO("error: " << outcome.errorMessage.toStdString());
    INFO("buffer events: " << bufferEvents.join(QStringLiteral(" | ")).toStdString());
    ...
    // The load-bearing assertion: the buffer declared itself stalled, and
    // said which buffer and which side of it was waiting. Without this the
    // case cannot tell its own arm from curl's resume path, which reports a
    // failure of its own for the same trickle.
    const QString all = bufferEvents.join(QStringLiteral(" | "));
    CHECK(all.contains(QStringLiteral("buffer: input")));
    CHECK(all.contains(QStringLiteral("stall_timeout")));
}

TEST_CASE("The extract callbacks answer safely with no buffers behind them",
          "[extract][stall]")
{
    // Both libarchive callbacks and the queue push are reached from the
    // decompressor's thread, and a teardown can take the ring buffers out
    // from under them -- a cancel during extraction frees them while
    // libarchive is still mid-call. Each has a guard for that, and none of
    // the guards had run: every case that gets as far as these has buffers.
    class Bufferless : public DownloadExtractThread
    {
    public:
        using DownloadExtractThread::DownloadExtractThread;
        using DownloadExtractThread::_on_read;
        using DownloadExtractThread::_pushQueue;
        using DownloadExtractThread::_writeData;
        void dropBuffers() { _ringBuffer.reset(); _writeRingBuffer.reset(); }
        void markCancelled() { _cancelled = true; }
    };

    Bufferless dt{QByteArray("file:///nonexistent"), QByteArray(), QByteArray()};
    dt.dropBuffers();

    // Nothing to read from: answer zero bytes rather than dereference it.
    const void *buff = reinterpret_cast<const void *>(0x1);
    CHECK(dt._on_read(nullptr, &buff) == 0);
    CHECK(buff == nullptr);

    // And nothing to push into: return rather than queue against a buffer
    // that has gone.
    const char payload[] = "some decompressed bytes";
    CHECK_NOTHROW(dt._pushQueue(payload, sizeof(payload)));

    // A cancelled write reports zero to curl, which is how the transfer is
    // stopped from the outside.
    dt.markCancelled();
    CHECK(dt._writeData(payload, sizeof(payload)) == 0);
}

// ══════════════════════════════════════════════════════════════
// The card Imager has to mount itself
//
// extractMultiFileRun() first waits to see whether the desktop auto-mounted
// the partition it just wrote. When nothing does -- a headless machine, a
// distribution without udisks, or simply a card the automounter declined --
// it mounts the FAT partition itself, extracts into that, and unmounts and
// removes its own temporary mount point afterwards.

#if defined(MULTIFILE_PROBE_BINARY) && defined(Q_OS_LINUX)
...
// vfat cannot be mounted inside a user namespace, and nothing in
// extractMultiFileRun() cares which filesystem answered: it mounts, chdirs
// in, extracts, and unmounts. Imager asks for "-t vfat <partition>
// <folder>", so the folder is the last argument.
bool plantMountShim(const QString &binDir)
{
    const QString path = QDir(binDir).filePath(QStringLiteral("mount"));
    static const char script[] =
        "#!/bin/sh\n"
        "folder=\"\"\n"
        "while [ $# -gt 0 ]; do folder=\"$1\"; shift; done\n"
        "exec /usr/bin/mount -t tmpfs rpi-imager-fake \"$folder\"\n";
    if (!writeFile(path, QByteArray(script)))
        return false;
    return QFile(path).setPermissions(
        QFileDevice::ReadOwner | QFileDevice::WriteOwner | QFileDevice::ExeOwner |
        QFileDevice::ReadGroup | QFileDevice::ExeGroup |
        QFileDevice::ReadOther | QFileDevice::ExeOther);
}

// A "mount" that reports success and mounts nothing, taking the directory
// with it. Imager trusts the exit code, so it goes on to wait for the mount
// point to appear -- which is what a container whose sandbox lags behind the
// host does -- and then tries to change into a directory that is not there.
bool plantVanishingMountShim(const QString &binDir)
{
    const QString path = QDir(binDir).filePath(QStringLiteral("mount"));
    static const char script[] =
        "#!/bin/sh\n"
...
QString runMultiFileProbe(const QString &mode, const QString &archive,
                          const QString &device, const QString &binDir)
{
    QProcess p;
    ...
    p.start(QStringLiteral("unshare"),
            {QStringLiteral("-rm"), QStringLiteral("--propagation"),
             QStringLiteral("private"), QStringLiteral(MULTIFILE_PROBE_BINARY),
             mode, archive, device});
...
// A device node name ending in a digit takes the "p1" partition suffix --
// mmcblk0p1, nvme0n1p1 -- and the plain "1" otherwise, sda1. Only the second
// was ever exercised. The partition file exists so the wait for the kernel to
// publish the node is skipped; that wait has cases of its own.
bool plantFakeDevice(const QString &path)
{
    QFile disk(path);
    if (!disk.open(QIODevice::WriteOnly) || !disk.resize(64 * 1024 * 1024))
        return false;
    disk.close();
    return writeFile(path + QStringLiteral("p1"), QByteArray());
}

} // namespace

TEST_CASE("A multi-file archive is written to a card Imager mounts itself",
          "[extract][multifile][mount]")
{
    if (!haveTool(QStringLiteral("zip")))
        SKIP("zip is not installed, so no multi-file fixture can be built");
    if (!haveMountNamespaces())
        SKIP("unprivileged mount namespaces are unavailable, so no mount can "
             "be performed without privilege");
    ...
    REQUIRE(plantMountShim(binDir));

    const QString device = scratch.filePath(QStringLiteral("mmcblk0"));
    REQUIRE(plantFakeDevice(device));

    const QString out = runMultiFileProbe(QStringLiteral("ok"), archive, device, binDir);
    INFO("probe output: " << out.toStdString());
    REQUIRE_FALSE(out.isEmpty());

    // euid 0 inside the namespace is what puts ARCHIVE_EXTRACT_OWNER into the
    // extraction flags, so the check is part of what the case covers.
    CHECK(out.contains(QStringLiteral("EUID=0")));
    CHECK(out.contains(QStringLiteral("FINISHED=1")));
    CHECK(out.contains(QStringLiteral("SUCCEEDED=1")));
}

TEST_CASE("An archive that ends early leaves nothing behind on the card",
          "[extract][multifile][mount]")
{
    if (!haveTool(QStringLiteral("zip")))
        SKIP("zip is not installed, so no multi-file fixture can be built");
    ...
    ScratchDir scratch;
    // Two members in a subdirectory. The first is extracted whole; the second
    // runs out of bytes part way through, and the cleanup then has to remove
    // both the file that landed and the directory it landed in -- the
    // directory is not an archive entry, so it is only reachable through the
    // extracted file's own path.
    REQUIRE(QDir().mkpath(scratch.filePath(QStringLiteral("sub"))));
    REQUIRE(writeFile(scratch.filePath(QStringLiteral("sub/a.bin")),
                      imageOfSize(256 * 1024, 31)));
    REQUIRE(writeFile(scratch.filePath(QStringLiteral("sub/b.bin")),
                      imageOfSize(256 * 1024, 37)));

    const QString archive = scratch.filePath(QStringLiteral("short.zip"));
    REQUIRE(runTool(QStringLiteral("zip"),
                    {QStringLiteral("-q"), QStringLiteral("-0"), QStringLiteral("-r"),
                     archive, QStringLiteral("sub")},
                    QFileInfo(archive).absolutePath()));

    // Stored, not deflated, so the cut lands squarely in the second member's
    // data rather than anywhere the decoder could recover from.
    {
        QFile f(archive);
        REQUIRE(f.open(QIODevice::ReadWrite));
        const qint64 full = f.size();
        REQUIRE(full > 300 * 1024);
        REQUIRE(f.resize(full - (200 * 1024)));
        f.close();
    }
    ...
    const QString out = runMultiFileProbe(QStringLiteral("truncated"), archive, device, binDir);
    INFO("probe output: " << out.toStdString());
    REQUIRE_FALSE(out.isEmpty());

    CHECK(out.contains(QStringLiteral("FINISHED=1")));
    CHECK(out.contains(QStringLiteral("SUCCEEDED=0")));
    CHECK_FALSE(out.contains(QStringLiteral("ERROR=\n")));
}

TEST_CASE("A mount point that never appears is reported, not extracted into",
          "[extract][multifile][mount]")
{
    if (!haveTool(QStringLiteral("zip")))
        SKIP("zip is not installed, so no multi-file fixture can be built");
    ...
    const QString binDir = scratch.filePath(QStringLiteral("bin"));
    REQUIRE(QDir().mkpath(binDir));
    REQUIRE(plantVanishingMountShim(binDir));

    const QString device = scratch.filePath(QStringLiteral("mmcblk0"));
    REQUIRE(plantFakeDevice(device));

    // Extracting into the current directory instead would scatter the archive
    // over wherever Imager happened to be, so the refusal is the point: the
    // write is abandoned and the user is told which directory failed.
    const QString out = runMultiFileProbe(QStringLiteral("ok"), archive, device, binDir);
    INFO("probe output: " << out.toStdString());
    REQUIRE_FALSE(out.isEmpty());
    ...
    CHECK(out.contains(QStringLiteral("FINISHED=1")));
    CHECK(out.contains(QStringLiteral("SUCCEEDED=0")));
    CHECK(out.contains(QStringLiteral("ERROR=Error changing to directory")));
}

#endif // MULTIFILE_PROBE_BINARY && Q_OS_LINUX
