 ...
 *
 * Everything inside the forked child is invisible to coverage whatever is
 * done here -- it leaves by execv or _exit, neither of which writes counters
 * out. What this reaches is the FIFO, the fork, and the teardown, which is
 * where the interesting behaviour is: releasing the inhibit has to finish
 * promptly whatever the tool does, or closing the application hangs after a
...
{
    if (argc < 2) {
        std::fprintf(stderr, "usage: %s <tool> [args...]\n", argv[0]);
        return 2;
    }
