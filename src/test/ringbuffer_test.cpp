...
TEST_CASE("Each kind of stall is named in the log", "[ringbuffer]")
{
    // This string is what goes in the log when a write stops moving, and it
    // is the first thing anyone reads when working out whose fault the stall
    // was -- the card's or the network's. Getting the two the wrong way round
    // sends the reader after the wrong end of the pipe.
    CHECK(std::string(RingBuffer::stallTypeToString(RingBuffer::StallType::None)) == "None");

    const std::string producer =
        RingBuffer::stallTypeToString(RingBuffer::StallType::ProducerStall);
    const std::string consumer =
        RingBuffer::stallTypeToString(RingBuffer::StallType::ConsumerStall);

    // A producer stall means the buffer is full because the disk cannot take
    // it; a consumer stall means it is empty because the download cannot fill
    // it. Naming them apart is the whole point.
    CHECK(producer.find("disk") != std::string::npos);
    CHECK(consumer.find("download") != std::string::npos);
    CHECK(producer != consumer);

    // An enumerator from the future still produces something printable
    // rather than reading off the end of the switch.
    CHECK(std::string(RingBuffer::stallTypeToString(
              static_cast<RingBuffer::StallType>(99))) == "Unknown");
}
