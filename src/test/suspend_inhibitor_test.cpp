...
TEST_CASE("An inhibit that cannot get its control FIFO leaves nothing behind",
          "[suspend][inhibit]")
{
    REQUIRE_NAMESPACES();

    // Two failures at once, because arranging either needs the same squeeze.
    // The FIFO's name is read from /dev/urandom, so a stale one cannot be
    // planted in advance -- but with no descriptor to spare that read falls
    // back to a name derived from the pid, which can be. So the first
    // attempt hits a name that is already taken (the FIFO a crashed run left
    // in /run), and the name it retries with cannot be opened, because there
    // is no descriptor for it either.
    const ProbeResult r = runProbe({QStringLiteral("--squeeze"), QStringLiteral("sh"),
                                    QStringLiteral("-c"), QStringLiteral("read x")});
    REQUIRE(r.ran);
    INFO("probe said:\n" << r.out.toStdString());

    // The collision was actually planted; without it the first attempt
    // simply succeeds and this tests nothing.
    REQUIRE(r.value(QStringLiteral("PLANTED")) == 1);

    // The planted one, and nothing else: no half-made FIFO from the attempt
    // that could not be opened.
    CHECK(r.value(QStringLiteral("FIFOS_DURING")) == 1);
    CHECK(r.value(QStringLiteral("FIFOS_AFTER")) == 1);

    // And no process was left waiting on a FIFO that was never opened.
    CHECK(r.value(QStringLiteral("CHILDREN_DURING")) == 0);
    CHECK(r.value(QStringLiteral("CHILDREN_AFTER")) == 0);
}
