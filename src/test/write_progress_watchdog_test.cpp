 ...
 *
 * The ladder it climbs -- poll, reduce queue depth, drain and hot-swap to
 * sync, restart, give up -- is minutes long by design, so the timings here
 * are scaled down by the subclass below. The ordering and the thresholds
 * relative to each other are what these cases pin; the absolute values are
 * timeout_utils.h's business.
 *
 * The thread being watched is a DownloadThread subclass that reports whatever
 * progress the case wants and records what recovery was asked of it. Nothing
 * is downloaded and nothing is written.
 */
