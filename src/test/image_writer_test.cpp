...
// QT_DBUS_LIB is not enough on its own: the embedded target still links
// Qt6DBus, but linux/Platform.cmake leaves urihandler_dbus.cpp out of the
// build, so the declaration would resolve and the definition would not.
#if defined(Q_OS_LINUX) && defined(QT_DBUS_LIB) && !defined(BUILD_EMBEDDED)
...
int main(int argc, char *argv[])
{
    ...
    // Set but empty: no download counter is reported. Every write here is
    // fabricated, and the counter is gated on the default OS list, which a
    // test satisfies -- so without this the suite posts one to the
    // production endpoint on every run.
    qputenv("RPI_IMAGER_TELEMETRY_URL", QByteArray());
    // Belt and braces for the other endpoint that talks to Raspberry Pi.
    // A case that forgets to redirect it would otherwise post to the live
    // Connect management API with whatever key happens to be in settings.
...
#ifdef Q_OS_MACOS
TEST_CASE("A device named in the environment is only taken if it is an image",
          "[imagewriter][device][macos]")
{
    using namespace rpi_imager::testing;

    // /dev/rdiskN is what a real disk is called too, so the name settles
    // nothing here the way /dev/loopN does on Linux. Pointed at the system
    // disk, the variable must be refused -- nothing is written either way,
    // and that is the point.
    qputenv("RPI_IMAGER_TEST_BLOCK_DEVICE", "/dev/disk0");
    {
        TestBlockDevice refused(8);
        CHECK(refused.path() != QStringLiteral("/dev/disk0"));
        CHECK(refused.path() != QStringLiteral("/dev/rdisk0"));
    }
    qunsetenv("RPI_IMAGER_TEST_BLOCK_DEVICE");

    // And the other direction, because a guard that refuses everything would
    // look just as safe while quietly breaking the variable.
    LoopDevice owned(8);
    if (!owned.isReady())
        SKIP("hdiutil could not attach an image to offer");
    qputenv("RPI_IMAGER_TEST_BLOCK_DEVICE", owned.path().toLatin1());
    {
        TestBlockDevice accepted(8);
        CHECK(accepted.path() == owned.path());
    }
    qunsetenv("RPI_IMAGER_TEST_BLOCK_DEVICE");
}
#endif
...
// NetworkManagerApi is desktop-only: the embedded target is built without
// DBus and linux/Platform.cmake stubs it out, so referring to it here made
// image_writer_test unbuildable in that configuration entirely. Guarded to
// match the source rather than dropped: the desktop build is where these
// credential decisions are made.
...
// ══════════════════════════════════════════════════════════════
// Reading this machine's wireless passphrase out of NetworkManager
//
// getPSK() is the other half of the credential path tested above: where
// getPSKForSSID() decides whether to answer, this is what actually reads
// the passphrase off the system bus. On a developer machine the real
// NetworkManager answers and then refuses GetSecrets for want of
// privilege, so the walk stopped at the same guard every time and neither
// the four rejection arms nor the arm that hands the passphrase over had
// ever run.

#if defined(NM_PSK_PROBE_BINARY)

namespace {

struct PskProbeResult
{
    bool ran = false;
    QString psk;
    QString diagnostics;
...
PskProbeResult runPskProbe(const QString &mode)
{
    PskProbeResult out;

    if (!QFileInfo::exists(QStringLiteral("/usr/bin/dbus-run-session")))
        return out;

    QProcess p;
    // The same bus for both: the code asks for the system bus, and
    // DBUS_SYSTEM_BUS_ADDRESS is what decides where that is.
    p.setProgram(QStringLiteral("/usr/bin/dbus-run-session"));
    p.setArguments({QStringLiteral("--"), QStringLiteral("/bin/sh"), QStringLiteral("-c"),
                    QStringLiteral("DBUS_SYSTEM_BUS_ADDRESS=\"$DBUS_SESSION_BUS_ADDRESS\" "
                                   "exec %1 %2")
                        .arg(QStringLiteral(NM_PSK_PROBE_BINARY), mode)});
    p.start();
    ...
    out.diagnostics = stdOut + QString::fromUtf8(p.readAllStandardError());
    for (const QString &line : stdOut.split(QLatin1Char('\n'))) {
        if (line.startsWith(QStringLiteral("PSK="))) {
            out.ran = true;
            out.psk = line.mid(4).trimmed();
        }
    }
    return out;
}
...
TEST_CASE("The passphrase is read off the bus when a connection has one", "[wlan][nm]")
{
    const PskProbeResult out = runPskProbe(QStringLiteral("found"));
    if (!out.ran)
        SKIP("no dbus-run-session, so no bus to put a NetworkManager on");

    INFO(out.diagnostics.toStdString());
    CHECK(out.psk == QStringLiteral("hunter2"));
}

TEST_CASE("A machine with no NetworkManager yields no passphrase", "[wlan][nm]")
{
    // Neither an error nor a guess: the wizard offers the "use this
    // network" option on the strength of the answer, and an invented one
    // puts an empty passphrase into an image.
    const PskProbeResult out = runPskProbe(QStringLiteral("nonm"));
    if (!out.ran)
        SKIP("no dbus-run-session, so no bus to put a NetworkManager on");

    INFO(out.diagnostics.toStdString());
    CHECK(out.psk.isEmpty());
}

TEST_CASE("Connections that cannot answer are stepped over, not stopped at",
          "[wlan][nm]")
{
    // Four ways a connection in the list has nothing to give: it names an
    // object that is not there, it names no settings, its settings object
    // is gone, or GetSecrets answers without a passphrase -- a wired
    // connection, which is in the list like any other. Each must be
    // skipped rather than end the walk.
    for (const QString &mode : {QStringLiteral("noactive"), QStringLiteral("nosettings"),
                                QStringLiteral("settingsgone"), QStringLiteral("nosecrets")}) {
        const PskProbeResult out = runPskProbe(mode);
        if (!out.ran)
            SKIP("no dbus-run-session, so no bus to put a NetworkManager on");
        INFO("mode " << mode.toStdString() << ": " << out.diagnostics.toStdString());
        CHECK(out.psk.isEmpty());
    }
}

TEST_CASE("A wired connection ahead of the wireless one does not hide it",
          "[wlan][nm]")
{
    // The guard that matters most. Machines are commonly on both, and
    // ethernet is usually first in the list; a walk that gave up on the
    // first connection with no passphrase would report that this machine
    // has no wireless credentials at all.
    const PskProbeResult out = runPskProbe(QStringLiteral("secondwins"));
    if (!out.ran)
        SKIP("no dbus-run-session, so no bus to put a NetworkManager on");
...
#endif  // NM_PSK_PROBE_BINARY

#endif  // !BUILD_EMBEDDED

// Outside the guard: WlanCredentials has a stub implementation on the
// embedded target, so the single-instance rule still holds there.
          ...
          "[imagewriter][locale]")
{
    ...
    // touchscreen -- which is why the assertion is guarded and the calls
    // above are not. hasMouse() is !isEmbeddedMode() || /dev/input/mouse0,
    // and a linuxfb Pi commonly has neither.
#if !defined(BUILD_EMBEDDED)
    CHECK(w.hasMouse());
#endif
}
...
#endif // Q_OS_LINUX && QT_DBUS_LIB && !BUILD_EMBEDDED
...
// Desktop-only by its own title: an embedded build does know which board it
// is on, so getHardwareName() is rightly not empty there.
#if !defined(BUILD_EMBEDDED)
...
// ---------------------------------------------------------------------------
// The cache as the background verifier leaves it.
//
// ImageWriter starts CacheManager's background operations in its constructor,
// so by the time a returning user presses Write the cache has usually already
// been checked. startWrite() reads that verdict and decides on the strength
// of it: use the cached copy, or throw it away and fetch again. Neither of
// those two branches had ever run -- every test reached Write before the
// verifier had finished, which is the third branch, the one that waits.

namespace {

// Reaches the cache the way the application does, and waits for the verdict
// the background verifier leaves -- which is the state startWrite() reads.
class CacheAwareWriter : public ImageWriter
{
public:
    explicit CacheAwareWriter(QObject *parent = nullptr) : ImageWriter(parent) {}

    using ImageWriter::_cacheManager;

    // Verify `hash` against the cache and pump until the answer is in, so the
    // case that follows starts from a settled verdict rather than a race.
    bool settleVerification(const QByteArray &hash, int timeoutMs = 30000)
    {
        QEventLoop loop;
        QObject context;
        bool done = false;
        QObject::connect(_cacheManager, &CacheManager::cacheVerificationComplete,
                         &context, [&](bool) { done = true; loop.quit(); });
        QTimer guard;
        guard.setSingleShot(true);
        QObject::connect(&guard, &QTimer::timeout, &loop, &QEventLoop::quit);
        guard.start(timeoutMs);

        _cacheManager->startVerification(hash);
        loop.exec();
        return done && _cacheManager->getCacheStatus().verificationComplete;
    }

    bool cacheThoughtValid() const { return _cacheManager->getCacheStatus().isValid; }
};

} // namespace

TEST_CASE("A cache already checked in the background is written without checking it again",
          "[imagewriter][cache]")
{
    // The returning user: the image is on disk from last time, the verifier
    // has been through it while the window was open, and pressing Write
    // should go straight to the card. Re-verifying a couple of gigabytes here
    // is a delay with nothing behind it, and re-downloading is worse.
    CacheFixture fx;
    CacheAwareWriter w(nullptr);
    w.setVerifyEnabled(false);
    w.setSrc(fx.sourceUrl(), 0, CacheFixture::kSize, sha256HexOf(fx.cacheBytes()));
    w.setCustomCacheFile(fx.cachePath(), sha256HexOf(fx.cacheBytes()));
    w.setDst(fx.target(), CacheFixture::kSize);
    ...
    REQUIRE(w.settleVerification(sha256HexOf(fx.cacheBytes())));
    REQUIRE(w.cacheThoughtValid());
    ...
    INFO("errors: " << out.errors.join(QStringLiteral(" | ")).toStdString());
    CHECK(out.succeeded);
    // Not checked a second time: the verdict was already in.
    CHECK(started.isEmpty());
    // And it is the cached copy that reached the card, not a fresh fetch.
    CHECK(fx.targetBytes() == fx.cacheBytes());
}

TEST_CASE("A cache the background check rejected is not the thing that gets written",
          "[imagewriter][cache]")
{
    // The same returning user, but the file on disk no longer hashes to what
    // it claims -- a half-finished download from a session that was killed, or
    // bit-rot on the disk it was cached to. The verdict is already in and it
    // is "no", so pressing Write has to discard it and fetch the image again.
    //
    // Writing it anyway is the failure that matters: it is a successful-looking
    // write of an image the machine already knows is corrupt, and the user
    // finds out when the board will not boot.
    CacheFixture fx;
    CacheAwareWriter w(nullptr);
    w.setVerifyEnabled(false);

    // The cache claims to be the source and is not: it holds cacheBytes.
    const QByteArray sourceHash = sha256HexOf(fx.sourceBytes());
    w.setSrc(fx.sourceUrl(), 0, CacheFixture::kSize, sourceHash);
    w.setCustomCacheFile(fx.cachePath(), sourceHash);
    w.setDst(fx.target(), CacheFixture::kSize);
    REQUIRE(w.readyToWrite());

    REQUIRE(w.settleVerification(sourceHash));
    REQUIRE_FALSE(w.cacheThoughtValid());

    const WriteOutcome out = runWrite(w);

    INFO("errors: " << out.errors.join(QStringLiteral(" | ")).toStdString());
    CHECK(out.succeeded);
    // The source, fetched again -- and emphatically not the rejected cache.
    CHECK(fx.targetBytes() == fx.sourceBytes());
    CHECK(fx.targetBytes() != fx.cacheBytes());
}

TEST_CASE("Whether a native file chooser is on offer is answerable", "[imagewriter]")
{
    // QML asks this before deciding which chooser to put in front of the
    // user: the platform's own, or the one drawn in QML. It is called during
    // page construction, so it has to answer on a writer that has not been
    // told anything yet rather than depend on a selection being made.
    ImageWriter w(nullptr);
    bool answered = false;
    CHECK_NOTHROW(answered = w.nativeFileDialogAvailable());
    // Either answer is right for the build; what matters is that asking is
    // safe and the answer is stable.
    CHECK(w.nativeFileDialogAvailable() == answered);
}

// ---------------------------------------------------------------------------
// Saving the performance data.
//
// The button that writes out what the last write actually did -- how long
// each phase took, what the card managed, where the time went. It is the
// first thing anyone is asked for when a write was slow, so it has to refuse
// clearly when there is nothing to save rather than produce an empty file
// that looks like an answer.
// ---------------------------------------------------------------------------

TEST_CASE("There is nothing to export before anything has been written",
          "[imagewriter][perf]")
{
    QTemporaryDir dir;
    REQUIRE(dir.isValid());
    const QString out = QDir(dir.path()).filePath(QStringLiteral("perf.json"));

    ImageWriter w(nullptr);

    // Both entry points refuse: the one that would put a file chooser in
    // front of the user, and the one QML calls once a path has been picked.
    CHECK_FALSE(w.exportPerformanceData());
    CHECK_FALSE(w.exportPerformanceDataToFile(out));

    // And refusing means refusing: no file, rather than an empty one that
    // would be sent on to someone as though it were a report.
    CHECK_FALSE(QFile::exists(out));
}

TEST_CASE("What a write did can be saved once it has been done",
          "[imagewriter][perf]")
{
    WriteFixture fx;
    ImageWriter w(nullptr);
    ...
    QTemporaryDir dir;
    REQUIRE(dir.isValid());
    const QString path = QDir(dir.path()).filePath(QStringLiteral("perf.json"));

    REQUIRE(w.exportPerformanceDataToFile(path));
    REQUIRE(QFile::exists(path));
    ...
    // Readable as JSON rather than however it happened to be assembled --
    // this file is fed to tooling, not read by eye.
    QJsonParseError err{};
    const QJsonDocument doc = QJsonDocument::fromJson(written, &err);
    INFO("parse error: " << err.errorString().toStdString());
    CHECK(err.error == QJsonParseError::NoError);
    CHECK_FALSE(doc.isNull());
...
TEST_CASE("Exporting to somewhere unwritable is reported, not silently lost",
          "[imagewriter][perf]")
{
    // A path the user picked that cannot be written -- a directory gone, or a
    // read-only volume. Returning true here would tell them their data was
    // saved somewhere it is not.
    WriteFixture fx;
    ImageWriter w(nullptr);
    w.setVerifyEnabled(false);
    w.setSrc(fx.sourceUrl(), 0, WriteFixture::kSize);
    ...
    CHECK_FALSE(w.exportPerformanceDataToFile(
        QStringLiteral("/proc/self/no/such/place/perf.json")));
}

TEST_CASE("A file the user picked is read, and one that is not there is not invented",
          "[imagewriter]")
{
    // How a chosen file reaches the wizard -- an SSH public key, a
    // cloud-init fragment. Every answer here goes on to be written into the
    // customisation, so "could not read it" and "it was empty" have to be
    // the same harmless answer rather than an exception or a partial read.
    QTemporaryDir dir;
    REQUIRE(dir.isValid());
    ImageWriter w(nullptr);

    SECTION("the contents come back, without the trailing newline") {
        const QString path = QDir(dir.path()).filePath(QStringLiteral("id_ed25519.pub"));
        QFile f(path);
        REQUIRE(f.open(QIODevice::WriteOnly));
        f.write("ssh-ed25519 AAAAC3Nz nobody@example\n\n");
        f.close();

        // Trimmed, because a key with a trailing blank line pasted into
        // authorized_keys is a key that does not work.
        CHECK(w.readFileContents(path) ==
              QStringLiteral("ssh-ed25519 AAAAC3Nz nobody@example"));
    }

    SECTION("a path that is not there reads as nothing") {
        CHECK(w.readFileContents(QDir(dir.path()).filePath(QStringLiteral("gone"))).isEmpty());
    }

    SECTION("a directory reads as nothing rather than as its listing") {
        CHECK(w.readFileContents(dir.path()).isEmpty());
    }

    SECTION("no path at all is not an error") {
        // QML calls this with whatever the chooser left behind, which is an
        // empty string when the user cancelled.
        CHECK(w.readFileContents(QString()).isEmpty());
    }
}

TEST_CASE("Cancel is answered while the cached image is still being checked",
          "[imagewriter][cache]")
{
    // Between pressing Write and the write starting there is a window where
    // the cached copy is being hashed, and on a large image that is many
    // seconds with a progress bar the user cannot otherwise get out of.
    // Cancel there has to be answered by the verification path -- nothing
    // has been started yet for the ordinary cancel to stop, so falling
    // through would leave the state at Cancelling for good, and startWrite's
    // re-entry guard then silently ignores every write from then on.
    CacheFixture fx;
    ImageWriter w(nullptr);
    w.setVerifyEnabled(false);
    w.setSrc(fx.sourceUrl(), 0, CacheFixture::kSize, sha256HexOf(fx.cacheBytes()));
    ...
    w.startWrite();
    REQUIRE(started.count() == 1);

    w.cancelWrite();

    // The UI is told the check is over, so the "checking cached image" state
    // comes down rather than staying on screen.
    CHECK(finished.count() == 1);

    // And the writer is usable again afterwards: a cancel that left the state
    // at Cancelling would make this second attempt do nothing at all, with
    // no error to show for it.
    const WriteOutcome again = runWrite(w);
    INFO("errors: " << again.errors.join(QStringLiteral(" | ")).toStdString());
    CHECK((again.succeeded || again.failed));
}

TEST_CASE("Forcing IPv4 is remembered, and only acted on when it changes",
          "[imagewriter]")
{
    // The escape hatch for a network where IPv6 resolves but does not carry
    // traffic -- the OS list fetch appears to hang and nothing explains why.
    // Turning it on reconfigures libcurl for every later transfer, so setting
    // it to what it already is must not churn that configuration underneath
    // a fetch that is already running.
    ImageWriter w(nullptr);
    const bool initial = w.getDebugIPv4Only();

    w.setDebugIPv4Only(!initial);
    CHECK(w.getDebugIPv4Only() == !initial);

    // Idempotent: the same value again is a no-op, not a second reconfigure.
    w.setDebugIPv4Only(!initial);
    CHECK(w.getDebugIPv4Only() == !initial);
...
TEST_CASE("A platform that cannot draw a title bar says so", "[imagewriter]")
{
    // The wizard draws its own close and minimise buttons when the platform
    // has no decorations of its own, and must not when it has -- otherwise
    // the user gets two sets, or none. The suite runs offscreen, which is one
    // of the platforms with none.
    ImageWriter w(nullptr);
    CHECK_FALSE(w.hasWindowDecorations());
}

// ══════════════════════════════════════════════════════════════
// Writing to a device that is not an ordinary block device
//
// startWrite() dispatches on choosePath(). Two of its four arms -- the
// fastboot device already in fastboot mode, and the rpiboot device that has
// to be sideloaded first -- had no test at all, so neither the dispatch nor
// the thread setup behind it had ever run: which signals get wired to
// PerformanceStats, whether the Connect organisation key is forwarded,
// whether a debug gadget override reaches the thread.

TEST_CASE("A fastboot device that is not there is reported rather than hung on",
          "[imagewriter][fastboot]")
{
    WriteFixture fx;
    ImageWriter w(nullptr);
    w.setVerifyEnabled(false);
    ...
    // The id shape the picker produces: fastboot id, then the block device
    // inside it after a slash.
    w.setFastbootDevice(QStringLiteral("0001-fastboot/mmcblk0"), WriteFixture::kSize);
    CHECK(w.isFastbootDevice());

    // The status message is what distinguishes the paths. Asserting on the
    // error text does not: the device id itself contains "fastboot", so a
    // substring match passed even with the dispatch branch disabled.
    QStringList status;
    QObject::connect(&w, &ImageWriter::preparationStatusUpdate, &w,
                     [&status](const QVariant &s) { status << s.toString(); });

    const WriteOutcome out = runWrite(w, 120000);
    INFO("errors: " << out.errors.join(QStringLiteral(" | ")).toStdString());
    INFO("status: " << status.join(QStringLiteral(" | ")).toStdString());
    CHECK(out.failed);
    CHECK(status.join(QStringLiteral(" ")).contains(QStringLiteral("Starting fastboot flash")));
}

TEST_CASE("An rpiboot device that is not there is reported rather than hung on",
          "[imagewriter][rpiboot]")
{
    WriteFixture fx;
    ImageWriter w(nullptr);
    ...
    // rpiboot://bus:addr:port.path:pid, and 2712 so the chip generation is
    // taken from the URI rather than left at the BCM2711 default.
    w.setRpibootDevice(QStringLiteral("rpiboot://1:9:2.4:2712"),
                       QStringLiteral("mmcblk0"));
    CHECK(w.isRpibootDevice());
    ...
    INFO("status: " << status.join(QStringLiteral(" | ")).toStdString());
    CHECK(out.failed);
    // Same reasoning as above: the status message is what says this went
    // down the rpiboot arm rather than the ordinary write.
    CHECK(status.join(QStringLiteral(" ")).contains(QStringLiteral("Preparing device for imaging")));
    // And the USB bus is what failed, not the image or the target.
    CHECK(out.errors.join(QStringLiteral(" ")).contains(QStringLiteral("USB")));
}

// cancelWrite() has an arm per thread kind, and the two non-block-device
// arms had never run. Cancelling is called immediately after starting,
// before the event loop turns, so the thread pointer is certainly still set
// -- the branch is taken deterministically rather than raced for.
TEST_CASE("Cancelling a fastboot write stops it and says so", "[imagewriter][fastboot]")
{
    WriteFixture fx;
    ImageWriter w(nullptr);
    w.setVerifyEnabled(false);
    ...
    QEventLoop loop;
    QObject::connect(&w, &ImageWriter::cancelled, &loop, [&] { cancelled = true; loop.quit(); });
    QObject::connect(&w, &ImageWriter::error, &loop, [&] { loop.quit(); });

    bool timedOut = false;
    QTimer guard;
    guard.setSingleShot(true);
    QObject::connect(&guard, &QTimer::timeout, &loop, [&] { timedOut = true; loop.quit(); });
    guard.start(60000);
    ...
    // Either it reported the cancel or it had already failed for want of a
    // device; what must not happen is the wizard sitting there for ever.
    INFO("cancelled: " << cancelled);
    CHECK_FALSE(timedOut);
}

TEST_CASE("Cancelling an rpiboot write stops it and says so", "[imagewriter][rpiboot]")
{
    WriteFixture fx;
    ImageWriter w(nullptr);
    w.setVerifyEnabled(false);
    w.setSrc(fx.sourceUrl(), 0, WriteFixture::kSize);
    w.setRpibootDevice(QStringLiteral("rpiboot://1:9:2.4:2712"), QStringLiteral("mmcblk0"));
...
// The destructor stops the flash thread before anything else, because
// concurrent libusb_exit and libusb_init deadlock on macOS. Nothing had
// destroyed an ImageWriter with that thread still live: by the time an
// ordinary test ends the thread has finished and nulled its own pointer.
// Destroying without turning the event loop leaves it set.
TEST_CASE("Tearing down mid-fastboot-write stops the thread rather than deadlocking",
          "[imagewriter][fastboot]")
{
    WriteFixture fx;
    {
        ...
        w.setFastbootDevice(QStringLiteral("0001-fastboot/mmcblk0"), WriteFixture::kSize);
        w.startWrite();
        // Destroyed here with the thread still running. Returning at all is
        // the assertion: the alternative is a hang at teardown, which on a
        // user's machine is Imager refusing to close.
    }
    SUCCEED("the destructor returned with a flash thread still in flight");
}

// The .json extension fix-up and the default filename. The export itself
// is covered elsewhere; these two are what decide whether the file a user
// sends to support is one any tool will open.
TEST_CASE("An exported report is named so that it opens", "[imagewriter][perf]")
{
    QTemporaryDir dir;
    REQUIRE(dir.isValid());

    SECTION("a filename with no extension is given one")
    {
        WriteFixture fx;
        ImageWriter w(nullptr);
        w.setVerifyEnabled(false);
        ...
        w.setDst(fx.target(), WriteFixture::kSize);
        const WriteOutcome out = runWrite(w, 120000);
        REQUIRE((out.succeeded || out.failed));

        const QString base = QDir(dir.path()).filePath(QStringLiteral("stats"));
        REQUIRE(w.exportPerformanceDataToFile(base));
        // Written as stats.json, not as a file called "stats".
        CHECK(QFile::exists(base + QStringLiteral(".json")));
        CHECK_FALSE(QFile::exists(base));
    }

    SECTION("the default filename is a .json name")
    {
        ImageWriter w(nullptr);
        const QString name = w.getPerformanceDataFilename();
        INFO("name: " << name.toStdString());
        CHECK(name.endsWith(QStringLiteral(".json")));
    }
}

// ══════════════════════════════════════════════════════════════
// The QML-facing helpers nothing had ever called
//
// ClipboardHelper, PlatformHelper and NativeFileDialog are registered as QML
// types and reached only from the interface, so no test binary had ever
// produced a single counter for them: they were absent from the coverage
// report rather than reported at zero, which is the more misleading of the
// two. Together that is a little over a hundred lines of shipping code --
// the paste that fills in a Wi-Fi passphrase, the settings that decide text
// size and motion for an accessibility user, and the dialog the performance
// export is saved through.
// ══════════════════════════════════════════════════════════════
...
TEST_CASE("The clipboard helper carries text both ways", "[helpers][clipboard]")
{
    // How a passphrase or a custom repository URL gets into the wizard
    // without being retyped.
    ClipboardHelper clip;

    int changes = 0;
    QObject::connect(&clip, &ClipboardHelper::clipboardChanged, [&changes] { ++changes; });

    clip.setText(QStringLiteral("correct horse battery staple"));
    CHECK(clip.getText() == QStringLiteral("correct horse battery staple"));
    CHECK(clip.hasText());

    // An empty clipboard is not "has text": the paste item is disabled on
    // that answer, and offering a paste that puts nothing anywhere is worse
    // than not offering it.
    clip.setText(QString());
    CHECK(clip.getText().isEmpty());
    CHECK_FALSE(clip.hasText());

    // refresh() exists because QClipboard::dataChanged is not delivered for
    // external changes on every Linux session, so the context menu asks
    // again as it opens.
    const int before = changes;
    clip.refresh();
    CHECK(changes == before + 1);
}

TEST_CASE("The platform helper answers what the interface asks of it",
          "[helpers][platform]")
{
    // Thin wrappers over PlatformQuirks, but they are what QML binds to, so
    // a wrapper returning the wrong quirk is a wrong answer on screen.
    PlatformHelper helper;

    CHECK(helper.hasNetworkConnectivity() == PlatformQuirks::hasNetworkConnectivity());
    CHECK(helper.fontDpiCorrection() == PlatformQuirks::fontDpiCorrection());
    CHECK(helper.prefersReducedMotion() == PlatformQuirks::prefersReducedMotion());
    CHECK(helper.isScrollInverted(true) == PlatformQuirks::isScrollInverted(true));
    CHECK(helper.isScrollInverted(false) == PlatformQuirks::isScrollInverted(false));
    CHECK_NOTHROW(helper.beep());
}

TEST_CASE("A text scale the user set overrides what the desktop reports",
          "[helpers][platform]")
{
    // Someone who has turned text up in Imager's own settings has done so
    ...
    PlatformHelper helper;
    const qreal platformValue = PlatformQuirks::detectTextScaleFactor();

    settings.remove(QStringLiteral("textScaleFactor"));
    settings.sync();
    CHECK(helper.textScaleFactor() == platformValue);

    settings.setValue(QStringLiteral("textScaleFactor"), 1.5);
    settings.sync();
    CHECK(helper.textScaleFactor() == 1.5);

    // Outside the range, and not a number at all: both fall back rather than
    // being clamped, because a stored value this far out is not a preference.
    for (const QVariant &bad : {QVariant(0.1), QVariant(9.0), QVariant(QStringLiteral("huge"))}) {
        settings.setValue(QStringLiteral("textScaleFactor"), bad);
        settings.sync();
        CHECK(helper.textScaleFactor() == platformValue);
    }

    if (saved.isValid())
        settings.setValue(QStringLiteral("textScaleFactor"), saved);
    else
        settings.remove(QStringLiteral("textScaleFactor"));
    settings.sync();
}

TEST_CASE("The accessibility observer is installed once, however often it is asked",
          "[helpers][platform]")
{
    // QAccessible::installActivationObserver keeps the bare pointer it is
    // given, so the observer is a single process-wide object that every
    // helper registers with. Registering the same helper twice would have it
    // notified twice for one change.
    PlatformHelper first;
    PlatformHelper second;

    CHECK(first.assistiveTechnologyActive() == QAccessible::isActive());
    CHECK(first.assistiveTechnologyActive() == QAccessible::isActive());
    CHECK(second.assistiveTechnologyActive() == QAccessible::isActive());
    CHECK_NOTHROW(second.assistiveTechnologyActive());
}

TEST_CASE("With native dialogs turned off every file dialog defers to QML",
          "[helpers][dialog]")
{
    // --qml-dialogs is there for sessions where the native dialog does not
    // work: no portal, a compositor that will not parent it, a kiosk. The
    // C++ side has no fallback of its own -- it answers empty and the QML
    // callsite opens its own -- so what matters is that it answers at all
    // rather than blocking on a dialog nothing will show.
    NativeFileDialog::setForceQmlDialogs(true);

    CHECK_FALSE(NativeFileDialog::areNativeDialogsAvailable());
    CHECK(NativeFileDialog::getOpenFileName(QStringLiteral("Open"), QDir::tempPath(),
                                            QStringLiteral("*.img")).isEmpty());
    CHECK(NativeFileDialog::getSaveFileName(QStringLiteral("Save"), QDir::tempPath(),
                                            QStringLiteral("*.json")).isEmpty());

    NativeFileDialog::setForceQmlDialogs(false);
}

TEST_CASE("Dialog timings are reported without the path in them", "[helpers][dialog]")
{
    // The timings go into the performance export, which users attach to bug
    // reports. A home directory path carries the account name, so the
    // directory is deliberately left out of the string.
    NativeFileDialog::TimingInfo info;
    info.pathParsingMs = 1;
    info.filterParsingMs = 2;
    info.panelCreationMs = 3;
    info.setDirectoryMs = 4;
    info.panelSetupMs = 5;
    info.totalBeforeShowMs = 15;
    info.userInteractionMs = 900;
    info.isSaveDialog = true;

    const QString rendered = fileDialogTimingToString(info);
    CHECK(rendered.contains(QStringLiteral("totalBeforeShow=15ms")));
    CHECK(rendered.contains(QStringLiteral("userInteraction=900ms")));
    CHECK(rendered.contains(QStringLiteral("isSave=true")));
    CHECK_FALSE(rendered.contains(QStringLiteral("directory")));

    // The last one recorded is readable afterwards; nothing has run a dialog
    // in this process, so it is the default rather than stale data.
    CHECK_NOTHROW(NativeFileDialog::lastTimingInfo());
}

TEST_CASE("The platform is asked whether it has a native dialog at all",
          "[helpers][dialog]")
{
    // With the override off the answer comes from the platform: on Linux
    // that is a session bus and an xdg-desktop-portal FileChooser. Under
    // ctest there is usually neither, and the point of the case is that
    // asking is cheap and does not block on a dialog server that is not
    // there -- which is exactly what it would do if the probe opened one.
    NativeFileDialog::setForceQmlDialogs(false);

    QElapsedTimer t;
    t.start();
    const bool available = NativeFileDialog::areNativeDialogsAvailable();
    INFO("native dialogs available: " << available);
...
//
// exportPerformanceData() is the button; exportPerformanceDataToFile() is
// what it calls once a path is known, and only the second had tests. The
// first decides where the path comes from -- a native dialog, or a signal
// asking QML to put one up -- and on a session with no portal the signal is
// the only route there is.
// ══════════════════════════════════════════════════════════════

TEST_CASE("With no native dialog the export asks QML for one", "[imagewriter][perf]")
{
    ExtractFixture fx(QStringLiteral("pattern-1MiB.img.xz"));
    ImageWriter w(nullptr);
    w.setVerifyEnabled(false);
    ...
    w.setDst(fx.target(), kFixturePayload);
    REQUIRE(runWrite(w).succeeded);
    REQUIRE(w.performanceStats()->hasImagingData());

    QString suggestedName;
    QString suggestedDir;
    QObject::connect(&w, &ImageWriter::performanceSaveDialogNeeded,
    ...
    NativeFileDialog::setForceQmlDialogs(true);
    // True because the request was handed on, not because a file was
    // written: QML owns the rest of it from here.
    CHECK(w.exportPerformanceData());
    NativeFileDialog::setForceQmlDialogs(false);

    // The name has to be one a file manager will accept and a person can
    // find again, and it has to end in .json or the save dialog will not
    // filter to it.
    CHECK(suggestedName.startsWith(QStringLiteral("rpi-imager-performance-")));
    CHECK(suggestedName.endsWith(QStringLiteral(".json")));
    CHECK_FALSE(suggestedName.contains(QLatin1Char('/')));
    CHECK_FALSE(suggestedDir.isEmpty());
}

TEST_CASE("An export with nothing but background events still goes out",
          "[imagewriter][perf]")
{
    // Exported before any write, or after a restart: there is data, but no
    // imaging session in it. Worth a warning, because a report like that
    // describes nothing the reporter thinks it does -- but not worth
    // refusing, because the background events are sometimes the point.
    ImageWriter w(nullptr);
    w.performanceStats()->recordEvent(PerformanceStats::EventType::FileDialogOpen,
                                      12, true, QStringLiteral("from a test"));
    REQUIRE(w.performanceStats()->hasData());
    REQUIRE_FALSE(w.performanceStats()->hasImagingData());

    bool asked = false;
    QObject::connect(&w, &ImageWriter::performanceSaveDialogNeeded,
                     [&asked](const QString &, const QString &) { asked = true; });
...
// The exported report as text, for asking whether an event is in it.
QString exportedReport(ImageWriter &w, const QString &path)
{
    REQUIRE(w.exportPerformanceDataToFile(path));
    QFile f(path);
    REQUIRE(f.open(QIODevice::ReadOnly));
...
TEST_CASE("What the watchdog did about a stall is in the report", "[imagewriter][perf]")
{
    QTemporaryDir dir;
    REQUIRE(dir.isValid());
    ...
    // startProgressPolling() is what builds the watchdog and wires it up.
    REQUIRE(QMetaObject::invokeMethod(&w, "startProgressPolling"));

    auto *watchdog = w.findChild<WriteProgressWatchdog *>();
    REQUIRE(watchdog != nullptr);

    // Called twice: the watchdog is built once and kept, so a second write
    // in the same session must not connect every handler again and record
    // each event twice.
    REQUIRE(QMetaObject::invokeMethod(&w, "startProgressPolling"));
    CHECK(w.findChildren<WriteProgressWatchdog *>().size() == 1);

    int warnings = 0;
    QObject::connect(&w, &ImageWriter::operationWarning, [&warnings](QVariant) { ++warnings; });

    emit watchdog->stallWarning(12, 3);
    emit watchdog->switchedToSyncMode(QStringLiteral("device stopped answering"));

    // The warning the user sees is the point of the hot swap: the write
    // carries on, more slowly, and they are told why.
    CHECK(warnings == 1);

    const QString report = exportedReport(w, dir.filePath(QStringLiteral("watchdog.json")));
    CHECK(report.contains(QStringLiteral("progressStall")));
    CHECK(report.contains(QStringLiteral("watchdogRecovery")));
    CHECK(report.contains(QStringLiteral("action=hotSwap")));
    CHECK(report.contains(QStringLiteral("device stopped answering")));
}

TEST_CASE("A watchdog timeout is reported as a failed write, not a quiet stop",
          "[imagewriter][perf]")
{
    // The end of the line: the device never came back and the write cannot
    // be recovered. This has to surface as an error the user sees, because
    // the alternative is a progress bar that simply stops.
    QTemporaryDir dir;
    REQUIRE(dir.isValid());
    ...
    QString reported;
    QObject::connect(&w, &ImageWriter::error, [&reported](QVariant msg) {
        reported = msg.toString();
    });

    emit watchdog->hardTimeout(QStringLiteral("no progress for 300s"));

    CHECK(reported.contains(QStringLiteral("no progress for 300s")));

    const QString report = exportedReport(w, dir.filePath(QStringLiteral("timeout.json")));
    CHECK(report.contains(QStringLiteral("action=hardTimeout")));
}

TEST_CASE("A restart the watchdog asks for is recorded and acted on",
          "[imagewriter][perf]")
{
    // The middle recovery: async writes stalled badly enough that the write
    // is worth starting again in sync mode, rather than hot-swapping
    // mid-stream. There is nothing in flight here, so what is pinned is that
    // the request reaches restartWrite() and is recorded as a recovery --
    // not that a write actually restarts.
    QTemporaryDir dir;
    REQUIRE(dir.isValid());
    ...
    CHECK_NOTHROW(emit watchdog->restartNeeded(QStringLiteral("async writes stalled")));

    const QString report = exportedReport(w, dir.filePath(QStringLiteral("restart.json")));
    CHECK(report.contains(QStringLiteral("action=restart")));
    CHECK(report.contains(QStringLiteral("async writes stalled")));
}

TEST_CASE("Everything the write thread reports about a bad write reaches the report",
          "[imagewriter][perf]")
{
    // These are the events that only happen when a write goes wrong, so a
    // suite in which every write succeeds never produces one. The handlers
    // are reached here by reporting them from the thread that would report
    // them, with a source that never delivers a byte so the write stays in
    // flight while they are sent.
    rpi_test::StallingHttpServer server(64 * 1024 * 1024);
    REQUIRE_HTTP_SERVER(server);

    QTemporaryDir dir;
    REQUIRE(dir.isValid());
    const QString target = dir.filePath(QStringLiteral("target.img"));
    {
        QFile t(target);
        REQUIRE(t.open(QIODevice::WriteOnly));
        REQUIRE(t.write(QByteArray(int(kFixturePayload), '\0')) == qint64(kFixturePayload));
    ...
    ImageWriter w(nullptr);
    w.setVerifyEnabled(false);
    // Announced as no bigger than the target: the size check runs before
    // the thread is built, and a source larger than the device never gets
    // one.
    w.setSrc(QUrl(QString::fromUtf8(server.urlFor(QStringLiteral("slow.img")))),
             kFixturePayload, kFixturePayload);
    w.setDst(target, kFixturePayload);
    ...
    QObject::connect(&w, &ImageWriter::error, [&problems](QVariant m) { problems << m.toString(); });
    QObject::connect(&w, &ImageWriter::preparationStatusUpdate,
                     [&problems](QVariant m) { problems << QStringLiteral("status: ") + m.toString(); });

    w.startWrite();

    DownloadThread *thread = nullptr;
    QElapsedTimer t;
    t.start();
    while (!thread && t.elapsed() < 10000) {
        QEventLoop loop;
        QTimer::singleShot(20, &loop, &QEventLoop::quit);
        loop.exec();
        thread = w.findChild<DownloadThread *>();
    }
    INFO("writer said: " << problems.join(QStringLiteral(" | ")).toStdString());
    REQUIRE(thread != nullptr);

    // Windows-only drive preparation: the handlers are built on every
    // platform, so a change to one is a change everywhere.
    emit thread->eventDriveUnmountVolumes(11, true);
    emit thread->eventDriveDiskClean(22, true);
    emit thread->eventDriveRescan(33, false);

    emit thread->eventPeriodicSync(44, true, 128u * 1024 * 1024);
    emit thread->eventDeviceClose(55, false);
    emit thread->eventDeviceIOTimeout(7, QStringLiteral("no completions"));
    emit thread->eventQueueDepthReduction(16, 8, 12);
    emit thread->eventDrainAndHotSwap(66, 4, true);
    emit thread->syncFallbackActivated(QStringLiteral("async I/O stalled"));
    emit thread->eventNetworkRetry(500, QStringLiteral("connection reset"));
    emit thread->eventWriteAfterSyncImpact(4000, 3000, 10);

    // The one of these the user is told about, rather than only the report.
    CHECK(warnings == 1);

    w.cancelWrite();
    QElapsedTimer settle;
    settle.start();
    while (w.findChild<DownloadThread *>() && settle.elapsed() < 15000) {
        QEventLoop loop;
        QTimer::singleShot(20, &loop, &QEventLoop::quit);
        loop.exec();
    }

    const QString report = exportedReport(w, dir.filePath(QStringLiteral("events.json")));

    for (const char *expected : {"driveUnmountVolumes", "driveDiskClean", "driveRescan",
                                 "periodicSync", "deviceClose", "deviceIOTimeout",
                                 "queueDepthReduction", "drainAndHotSwap",
                                 "syncFallbackActivated", "networkRetry",
                                 "writeAfterSyncImpact"}) {
        INFO("looking for " << expected);
        CHECK(report.contains(QString::fromLatin1(expected)));
    }

    // The metadata is the part that makes an event worth having: a queue
    // depth cut from 16 to 8 says something a bare count does not.
    CHECK(report.contains(QStringLiteral("depth=16->8")));
    CHECK(report.contains(QStringLiteral("pending=7")));
    CHECK(report.contains(QStringLiteral("at 128 MB")));
    CHECK(report.contains(QStringLiteral("impactPercent: 25")));
}

// ══════════════════════════════════════════════════════════════
// The handful of things QML hands ImageWriter, and what it does with them
//
// The window, the engine, the keychain answer, the read-only mount and the
// report of a listing that would not load are each a few lines called from
// one place in QML, and between them a hundred lines of imagewriter.cpp had
// never run. They are small, but the window one in particular is not
// cosmetic: it is what the Connect sign-in relies on to put the token
// prompt in front of somebody.
// ══════════════════════════════════════════════════════════════

#include <QQuickWindow>
#include <QQmlApplicationEngine>

TEST_CASE("The main window is taken from whatever QML hands over",
          "[imagewriter][window]")
{
    ImageWriter w(nullptr);
    ...
    SECTION("something that is not a window at all") {
        QObject notAWindow;
        CHECK_NOTHROW(w.setMainWindow(&notAWindow));
        // No window, so nothing to raise -- and in particular no call
        // through a null pointer.
        CHECK_NOTHROW(w.bringWindowToForeground());
    }

    SECTION("nothing") {
        CHECK_NOTHROW(w.setMainWindow(nullptr));
        CHECK_NOTHROW(w.bringWindowToForeground());
    }
}

TEST_CASE("The QML engine is given the icon provider it needs",
          "[imagewriter][window]")
{
    // Without this every image://icons/<url> in the OS list resolves to
    // nothing and the list draws with no logos at all.
    ImageWriter w(nullptr);

    QQmlApplicationEngine engine;
    CHECK_NOTHROW(w.setEngine(&engine));
    CHECK(engine.imageProvider(QStringLiteral("icons")) != nullptr);

    // And a null engine is not a crash: the CLI path constructs the writer
    // without one.
    CHECK_NOTHROW(w.setEngine(nullptr));
}

TEST_CASE("A native open dialog is not offered when there is none",
          "[imagewriter][dialog]")
{
    // The QML side asks for a native dialog first and puts up its own when
    // the answer is empty. Answering with a path here, or blocking, would
    // both be worse than saying nothing.
    ImageWriter w(nullptr);
    NativeFileDialog::setForceQmlDialogs(true);
    CHECK(w.getNativeOpenFileName(QStringLiteral("Choose"), QDir::tempPath(),
                                  QStringLiteral("*.img")).isEmpty());
    NativeFileDialog::setForceQmlDialogs(false);
}

TEST_CASE("The keychain answer is recorded either way", "[imagewriter]")
{
    // macOS asks before reading the saved Wi-Fi passphrase out of the
    // keychain, and the answer arrives from QML long after the asking code
    // has started waiting. Recording a refusal is as load-bearing as
    // recording consent: without it the wait runs to its timeout and the
    // wizard stalls on a question the user already answered.
    ImageWriter w(nullptr);

    int responses = 0;
    QObject::connect(&w, &ImageWriter::keychainPermissionResponseReceived,
                     [&responses] { ++responses; });

    CHECK_NOTHROW(w.keychainPermissionResponse(true));
    CHECK(responses == 1);
    CHECK_NOTHROW(w.keychainPermissionResponse(false));
    CHECK(responses == 2);
}

TEST_CASE("Mounting the source media read-only is refused without privilege",
          "[imagewriter]")
{
    // The USB-source path mounts what it finds read-only, so that reading an
    ...
    // mountReadOnly is protected so that a test can replace it; here the
    // real one is wanted.
    struct RealMount : ImageWriter {
        using ImageWriter::ImageWriter;
        using ImageWriter::mountReadOnly;
    };
    RealMount w(nullptr);
    const int rc = w.mountReadOnly(QStringLiteral("/dev/rpi-imager-no-such-device"),
                                   dir.path());
    INFO("mount returned " << rc);
    CHECK(rc != 0);
}

TEST_CASE("A sublist URL with no scheme is named in the warning", "[imagewriter]")
{
    // Local --repo files carry subitems_url entries, and a relative one is
    // the commonest mistake in writing them: it fetches nothing, and
    // without naming the problem the user sees an OS list that is simply
    // missing a section.
    ImageWriter w(nullptr);

    // Whatever the top-level state is before, a sublist failing must not
    // change it: the sections that did load still have to be shown.
    const bool unavailableBefore = w.isOsListUnavailable();

    CHECK(QMetaObject::invokeMethod(&w, "onOsListFetchError",
                                    Q_ARG(QString, QStringLiteral("could not resolve host")),
                                    Q_ARG(QUrl, QUrl(QStringLiteral("sublist.json")))));

    CHECK(w.isOsListUnavailable() == unavailableBefore);
}

#include "fastbootflashthread.h"

TEST_CASE("What a fastboot flash reports reaches the interface and the report",
          "[imagewriter][fastboot][perf]")
{
    // The fastboot path has its own progress wiring -- a separate set of
    // lambdas from the card path's, doing the same job for a board that is
    // written over USB. None of it had run: every fastboot test here fails
    // before the device is open, so the thread never reports progress and
    // the handlers that forward it were dead.
    //
    // Both halves matter. The interface's progress bar is one, and the
    // performance report is the other, and they are fed from the same two
    // signals by different handlers -- so one can be dropped without the
    // other noticing.
    WriteFixture fx;
    ImageWriter w(nullptr);
    w.setVerifyEnabled(false);

    // A bmap URL, which only the fastboot path uses: it is how a sparse
    // image is written without sending the empty parts over USB.
    w.setSrc(fx.sourceUrl(), 0, WriteFixture::kSize, QByteArray(), false,
             QString(), QString(), QByteArray(), QString(),
             QStringLiteral("https://example.invalid/os.bmap"));
    w.setFastbootDevice(QStringLiteral("0001-fastboot/mmcblk0"), WriteFixture::kSize);

    quint64 downloadNow = 0, writeNow = 0;
    QObject::connect(&w, &ImageWriter::downloadProgress, &w,
                     [&downloadNow](QVariant n, QVariant) { downloadNow = n.toULongLong(); });
    QObject::connect(&w, &ImageWriter::writeProgress, &w,
                     [&writeNow](QVariant n, QVariant) { writeNow = n.toULongLong(); });

    w.startWrite();

    // The thread is built and wired before it is started, and nothing has
    // run an event loop yet, so it is still here to report through.
    auto *flash = w.findChild<FastbootFlashThread *>();
    REQUIRE(flash != nullptr);

    emit flash->eventFastbootDeviceOpen(42, true, QStringLiteral("via usb"));
    emit flash->downloadProgress(1024, 4096);
    emit flash->writeProgress(2048, 4096);
    emit flash->writing();

    CHECK(downloadNow == 1024);
    CHECK(writeNow == 2048);
    // writing() is what moves the interface out of "preparing" and starts
    // the stall watchdog.
    // Read through the meta-object: the accessor is private, and QML
    // reads it as a property too.
    CHECK(w.property("writeState").value<ImageWriter::WriteState>() ==
          ImageWriter::WriteState::Writing);
    CHECK(w.findChild<WriteProgressWatchdog *>() != nullptr);

    // Let it fail the way it was always going to -- there is no board.
    const WriteOutcome out = runWrite(w, 120000);
    INFO("errors: " << out.errors.join(QStringLiteral(" | ")).toStdString());
    CHECK(out.failed);
    ...
    QTemporaryDir dir;
    REQUIRE(dir.isValid());
    const QString report = exportedReport(w, dir.filePath(QStringLiteral("fastboot.json")));
    CHECK(report.contains(QStringLiteral("fastbootDeviceOpen")));
    CHECK(report.contains(QStringLiteral("via usb")));
}

#include "downloadstatstelemetry.h"

// ══════════════════════════════════════════════════════════════
// Starting a second write in the same session
//
// startWrite() begins by taking down whatever the previous write left --
// the extract thread, or an rpiboot thread, or a fastboot one. Nothing had
// ever done two writes through one ImageWriter, so none of that teardown
// had run. It is not a rare path: writing two cards in a row is the
// ordinary thing to do with Imager, and a thread left behind from the first
// is one still holding a device the second is about to open.
// ══════════════════════════════════════════════════════════════

// ══════════════════════════════════════════════════════════════
// Writing a second card through the same writer
//
// This stalled for good, and writing two cards in a row is the ordinary
// thing to do with Imager. startWrite() let the previous thread go with
// deleteLater() but left its signals connected, so its queued finished()
// was still delivered once the caller was inside the next write. The
// handler then acted on whatever _thread named by then -- the new, running
// thread -- and destroyed it, leaving the state machine in Finalizing with
// nothing left to end it: no success(), no error(), for as long as anyone
// waited. What the user sees is a progress bar that stops with no message.
// ══════════════════════════════════════════════════════════════
TEST_CASE("A second card write through the same writer completes",
          "[imagewriter][write]")
{
    ImageWriter w(nullptr);
    w.setVerifyEnabled(false);
    ...
    WriteFixture first;
    w.setSrc(first.sourceUrl(), 0, WriteFixture::kSize);
    w.setDst(first.target(), WriteFixture::kSize);
    REQUIRE(w.readyToWrite());
    const WriteOutcome one = runWrite(w, 60000);
    INFO("first errors: " << one.errors.join(QStringLiteral(" | ")).toStdString());
    REQUIRE(one.succeeded);

    // A source and target of their own, so nothing here is shared with the
    // first write except the writer itself.
    WriteFixture second;
    w.setSrc(second.sourceUrl(), 0, WriteFixture::kSize);
    w.setDst(second.target(), WriteFixture::kSize);
    REQUIRE(w.readyToWrite());
    const WriteOutcome two = runWrite(w, 120000);
    INFO("second statuses: " << two.statuses.join(QStringLiteral(" | ")).toStdString());
    INFO("second errors: " << two.errors.join(QStringLiteral(" | ")).toStdString());
    CHECK(two.succeeded);
}

TEST_CASE("A second write takes down what the first one left", "[imagewriter][write]")
{
    WriteFixture fx;
    ImageWriter w(nullptr);
    w.setVerifyEnabled(false);

    // Deliberately not here: a second *card* write through the same writer.
    // It stalls -- the second run reports "Opening drive", "Zero'ing out",
    // "Opening image file", "Starting extraction", and then neither
    // success() nor error() ever arrives, for at least the two minutes the
    // harness waits. Not the fixture: a second source and target of their
    // own behave the same, so it is state left on the writer. A defect to
    // chase with a debugger rather than pin here, and a two-minute stall
    // is too expensive to leave in the suite as a marker.

    SECTION("after a write to a board that was not there") {
        // The rpiboot attempt fails, and leaves a thread behind. The next
        // write has to take it down before opening anything.
        w.setSrc(fx.sourceUrl(), 0, WriteFixture::kSize);
        w.setRpibootDevice(QStringLiteral("0001-rpiboot"), QStringLiteral("mmcblk0"));
        CHECK(runWrite(w, 120000).failed);

        w.setRpibootDevice(QString(), QString());
        w.setSrc(fx.sourceUrl(), 0, WriteFixture::kSize);
        w.setDst(fx.target(), WriteFixture::kSize);
        CHECK(runWrite(w).succeeded);
    }

    SECTION("after a fastboot write to a board that was not there") {
        w.setSrc(fx.sourceUrl(), 0, WriteFixture::kSize);
        w.setFastbootDevice(QStringLiteral("0001-fastboot/mmcblk0"), WriteFixture::kSize);
        CHECK(runWrite(w, 120000).failed);

        w.setFastbootDevice(QString(), 0);
        w.setSrc(fx.sourceUrl(), 0, WriteFixture::kSize);
        w.setDst(fx.target(), WriteFixture::kSize);
        CHECK(runWrite(w).succeeded);
    }
...
// Spin the loop until the write thread has gone. runWrite() cannot be used
// to wind a write down: it calls startWrite() itself, which would begin
// another one.
void settle(ImageWriter &w, int budgetMs = 20000)
{
    QElapsedTimer t;
    t.start();
    while (t.elapsed() < budgetMs && w.findChild<QThread *>() != nullptr) {
        QEventLoop loop;
        QTimer::singleShot(20, &loop, &QEventLoop::quit);
        loop.exec();
    }
...
TEST_CASE("The download counter goes where it is pointed, or nowhere",
          "[imagewriter][telemetry]")
{
    // The counter is gated on the user being on the default OS list, which
    // is exactly what a test looks like -- which is how every write in this
    // suite came to post one to the production endpoint. The endpoint is
    // now read from the environment, and set-but-empty means do not report.
    //
    // Both directions are pinned here, because the interesting failure is
    // the silent one: an override that is read but ignored looks identical
    ...
    SECTION("set but empty reports nothing") {
        ...
        w.startWrite();
        CHECK(w.findChild<DownloadStatsTelemetry *>() == nullptr);
        w.cancelWrite();
        settle(w);
    }

    SECTION("pointed somewhere, it reports there") {
        qputenv("RPI_IMAGER_TELEMETRY_URL", QByteArray("http://127.0.0.1:1/downloads"));
        ImageWriter w(nullptr);
        w.setVerifyEnabled(false);
        w.setSrc(QUrl(QStringLiteral("https://example.invalid/os.img")), 0,
                 WriteFixture::kSize);
        ...
        w.startWrite();
        // Port 1, so nothing is listening and the post fails -- which the
        // counter is meant to survive. What is pinned is that it was
        // dispatched at all.
        CHECK(w.findChild<DownloadStatsTelemetry *>() != nullptr);
        w.cancelWrite();
        settle(w);
...
TEST_CASE("Minting an organisation auth key reports what the server said",
          "[imagewriter][connect]")
{
    // The wizard calls this to mint a Connect key and writes the secret it
    // returns straight into the image's customisation, as though the user
    // had pasted a per-user token. Every arm of it was unreachable: it
    // built its registrar with no base URL, so the only test that could be
    // written for it was one that posted to the real management API.
    //
    // Three answers matter, and the two failures matter most: a key the
    // wizard shows as minted when it was not is a card written with no
    // credential on it, discovered only when the board fails to enrol.

    SECTION("with no organisation API key configured") {
        // The commonest case: Connect for Organisations is simply off.
        ImageWriter w(nullptr);
        QSettings settings;
        const QVariant saved = settings.value(QStringLiteral("connect_org_api_key"));
        settings.remove(QStringLiteral("connect_org_api_key"));
        settings.sync();

        const QVariantMap out = w.requestOrgAuthKey(QStringLiteral("a board"), 1);
        CHECK_FALSE(out.value(QStringLiteral("ok")).toBool());
        CHECK_FALSE(out.value(QStringLiteral("error")).toString().isEmpty());
        // No secret on a failure, not even an empty one to be written out.
        CHECK_FALSE(out.contains(QStringLiteral("secret")));

        if (saved.isValid())
            settings.setValue(QStringLiteral("connect_org_api_key"), saved);
        settings.sync();
    }

    SECTION("when the server cannot be reached") {
        // Port 1, so the POST fails at connect. The wizard has to be told,
        // rather than shown a blank key.
        qputenv("RPI_IMAGER_CONNECT_URL", QByteArray("http://127.0.0.1:1"));
        QSettings settings;
        const QVariant saved = settings.value(QStringLiteral("connect_org_api_key"));
        settings.setValue(QStringLiteral("connect_org_api_key"),
                          QStringLiteral("rpak_testkey"));
        settings.sync();

        ImageWriter w(nullptr);
        const QVariantMap out = w.requestOrgAuthKey(QStringLiteral("a board"), 1);

        INFO("error: " << out.value(QStringLiteral("error")).toString().toStdString());
        CHECK_FALSE(out.value(QStringLiteral("ok")).toBool());
        CHECK_FALSE(out.value(QStringLiteral("error")).toString().isEmpty());
        CHECK_FALSE(out.contains(QStringLiteral("secret")));
        ...
        if (saved.isValid())
            ...
            settings.remove(QStringLiteral("connect_org_api_key"));
        settings.sync();
        // Back to the guard main() set, not unset: a later case with a key
        // in settings would otherwise reach production.
        qputenv("RPI_IMAGER_CONNECT_URL", QByteArray("http://127.0.0.1:1"));
    }
}

TEST_CASE("The Connect base URL can be pointed somewhere other than production",
          "[imagewriter][connect]")
{
    // Both callers inside ImageWriter construct the registrar without a
    // base URL of their own, so without this there is no way to point
    // either at a stub -- and a staging deployment wants the same door.
    qunsetenv("RPI_IMAGER_CONNECT_URL");
    ConnectDeviceRegistrar production(QStringLiteral("k"), QString());
    const QString productionUrl = production.baseUrl();
    CHECK(productionUrl.contains(QStringLiteral("raspberrypi")));

    qputenv("RPI_IMAGER_CONNECT_URL", QByteArray("http://127.0.0.1:9/api"));
    ConnectDeviceRegistrar redirected(QStringLiteral("k"), QString());
    CHECK(redirected.baseUrl() == QStringLiteral("http://127.0.0.1:9/api"));

    // An explicit base URL still wins over the environment, so a caller
    // that threads one through is not quietly overridden.
    ConnectDeviceRegistrar explicitly(QStringLiteral("k"), QString(),
                                      QStringLiteral("http://127.0.0.1:8/other"));
    CHECK(explicitly.baseUrl() == QStringLiteral("http://127.0.0.1:8/other"));

    qputenv("RPI_IMAGER_CONNECT_URL", QByteArray("http://127.0.0.1:1"));
}

TEST_CASE("Tearing the writer down releases what the write was holding",
          "[imagewriter]")
{
    // Closing the window mid-write, and closing it after the language has
    // been changed. The destructor has to take down the suspend inhibitor
    // and the installed translator as well as the thread: leaving the
    // inhibitor behind keeps the machine awake indefinitely, and leaving
    // the translator installed hands QCoreApplication a dangling one.
    QTemporaryDir dir;
    REQUIRE(dir.isValid());

    {
        auto w = std::make_unique<ImageWriter>(nullptr);
        w->changeLanguage(QStringLiteral("Deutsch"));
        // Builds the suspend inhibitor, which is otherwise only created
        // once a write is under way.
        REQUIRE(QMetaObject::invokeMethod(w.get(), "startProgressPolling"));
        CHECK_NOTHROW(w.reset());
    }

    // Nothing left installed on the application: a translator outliving
    // its writer is one QCoreApplication would call into.
    CHECK_NOTHROW(QCoreApplication::translate("ImageWriter", "Verifying written data"));
}

TEST_CASE("Cancelling before the cache check finishes stops the check, not a write",
          "[imagewriter][write]")
{
    // Cancel pressed while the cached image is still being verified. There
    // is no write to stop yet, so the cancel has to reach the verification
    // instead -- otherwise the button does nothing and the wizard sits on
    // "Checking cached image" until the hash finishes.
    ImageWriter w(nullptr);

    QList<ImageWriter::WriteState> states;
    QObject::connect(&w, &ImageWriter::writeStateChanged, [&] {
        states << w.property("writeState").value<ImageWriter::WriteState>();
    });

    CHECK_NOTHROW(w.cancelWrite());

    // It went through Cancelling, whatever it settled on afterwards.
    REQUIRE_FALSE(states.isEmpty());
    CHECK(states.first() == ImageWriter::WriteState::Cancelling);
}

TEST_CASE("A board appearing in rpiboot mode is ignored unless the option is on",
          "[imagewriter][rpiboot]")
{
    // The drive poller reports every Compute Module it sees on the bus.
    // Acting on one without the user having turned rpiboot support on
    // would start a sideload nobody asked for, against a board that may
    // be plugged in for something else entirely.
    ImageWriter w(nullptr);

    bool started = false;
    QObject::connect(&w, &ImageWriter::preparationStatusUpdate,
                     [&started](QVariant) { started = true; });

    // Named, because Q_ARG is a macro and a braced list's comma splits it.
    const QList<uint8_t> portPath{1, 2};
    CHECK(QMetaObject::invokeMethod(&w, "onRpibootDeviceDetected",
                                    Q_ARG(QString, QStringLiteral("0001-rpiboot")),
                                    Q_ARG(uint8_t, uint8_t(1)),
                                    Q_ARG(uint8_t, uint8_t(4)),
                                    Q_ARG(QList<uint8_t>, portPath),
                                    Q_ARG(uint16_t, uint16_t(0x2712))));
    CHECK_FALSE(started);
}

TEST_CASE("Hardware tags come from the OS list, and a list without them is not fatal",
          "[imagewriter]")
{
    // The board chooser is built from the "imager.devices" block of the OS
    // list, and the writer starts with the bundled list already in hand --
    // so this succeeds without anything being fetched, which is what makes
    // the chooser usable before the network answers.
    ImageWriter w(nullptr);
    CHECK(w.createHardwareTags());

    // Through the base: the override is protected, and QML reads the model
    // through QAbstractItemModel's public interface anyway. The bundled
    // list carries no devices block of its own, so the chooser is empty
    // until a repository is fetched -- which is why this must not fail.
    auto *model = static_cast<QAbstractItemModel *>(w.getHWList());
    const int rows = model->rowCount();

    // Idempotent: the wizard rebuilds the tags whenever the list changes,
    // and a second pass that appended again would show every board twice.
    CHECK(w.createHardwareTags());
    CHECK(model->rowCount() == rows);
}

TEST_CASE("A cache that fails its check is not written; the download happens instead",
          "[imagewriter][cache][write]")
{
    // Cache verification finishing is what releases the write, and what it
    // decides is where the bytes come from. A cache that verified is read
    // off disk; one that did not has to be passed over and the image
    // fetched again -- writing it anyway is writing half a download to a
    // card, which is the failure the verification exists to prevent.
    //
    // The remote half had never run: every cached write in the suite has a
    // local source, so the branch that keeps a remote URL -- and the
    // download counter that goes with it -- was unreachable.
    QTemporaryDir served;
    REQUIRE(served.isValid());
    const QByteArray image = QByteArray(2 * 1024 * 1024, '\x5a');
    {
        QFile f(served.filePath(QStringLiteral("os.img")));
        REQUIRE(f.open(QIODevice::WriteOnly));
        REQUIRE(f.write(image) == image.size());
    }
    ...
    const QString cachePath = work.filePath(QStringLiteral("cached.img"));
    const QString target = work.filePath(QStringLiteral("target.img"));
    {
        // A cache file that is not the image: this is what a download
        // interrupted half way leaves behind.
        QFile c(cachePath);
        REQUIRE(c.open(QIODevice::WriteOnly));
        REQUIRE(c.write(QByteArray(2 * 1024 * 1024, '\xa5')) == image.size());
        QFile t(target);
        REQUIRE(t.open(QIODevice::WriteOnly));
        REQUIRE(t.write(QByteArray(2 * 1024 * 1024, '\0')) == image.size());
    }

    const QByteArray hash =
        QCryptographicHash::hash(image, QCryptographicHash::Sha256).toHex();

    // Pointed at a dead port rather than silenced, so the counter is
    // dispatched and its failure is survived -- which is the behaviour
    // that matters on the remote path.
    qputenv("RPI_IMAGER_TELEMETRY_URL", QByteArray("http://127.0.0.1:1/downloads"));

    ImageWriter w(nullptr);
    w.setVerifyEnabled(false);
    w.setSrc(QUrl(QString::fromUtf8(server.urlFor(QStringLiteral("os.img")))),
             image.size(), image.size(), hash);
    w.setCustomCacheFile(cachePath, hash);
    w.setDst(target, image.size());
    ...
    // The image, not the cache file that failed its check.
