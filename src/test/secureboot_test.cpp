...
TEST_CASE("SecureBoot extracts nothing from a partition that is not there", "[secureboot][fat]")
{
    // extractFatPartitionFiles is handed whatever the caller managed to open.
    // A null partition must come back as an empty set rather than a crash:
    // the caller goes on to sign what it got, and signing nothing is
    // recoverable in a way that dereferencing nothing is not.
    SecureBoot secureBoot;
    (void)secureBoot;

    CHECK(SecureBoot::extractFatPartitionFiles(nullptr).isEmpty());
}

TEST_CASE("SecureBoot skips a zero-length file and keeps the rest", "[secureboot][fat]")
{
    if (!rpi_test::haveFatFormatter())
        SKIP(rpi_test::noFatFormatterReason());
    ...
    // An empty config.txt is a thing users really produce -- touch it and
    // forget it. It must not take the rest of the boot partition with it.
    fat.writeFile(QStringLiteral("empty.txt"), QByteArray());
    fat.writeFile(QStringLiteral("config.txt"), "arm_64bit=1\n");
    dw.sync();
