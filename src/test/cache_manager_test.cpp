...
// Whether this machine can cache at all.
bool cachingIsPossible(qint64 downloadSize)
{
    // The embedded target caches nothing at all -- CacheManager is
    // constructed with cachingEnabled_(!isEmbeddedMode()), because the
    // device it runs on has no room to keep a compressed image beside the
    // one it is writing. Everything below describes desktop behaviour, so
    // on embedded there is nothing to describe rather than something
    // broken. Reached only now: the embedded tree did not build enough of
    // the suite to run these until this branch.
    if (::isEmbeddedMode())
        return false;
    ...
    if (dir.isEmpty())
        return false;
    QDir().mkpath(dir);
    const qint64 available = QStorageInfo(dir).bytesAvailable();
    return available > 0 &&
           (available - downloadSize) >= IMAGEWRITER_MINIMAL_SPACE_FOR_CACHING;
}
...
{
    // No space guard here, unlike the cases above: this one never asks
    // CacheManager to make a cache, it hands it one that already exists.
    clearCacheDir();
    clearCacheSettings();
...
{
    if (!cachingIsPossible(4096))
        SKIP("this target caches nothing, so there is no cache to describe");
    clearCacheDir();
    CacheManager manager;
