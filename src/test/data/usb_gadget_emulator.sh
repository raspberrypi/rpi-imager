...
#   vhci-hcd    a USB host controller implemented in software. It attaches a
#               usbip-exported device to this machine's USB tree.
#
# Pointing the second at the first loops the emulated device back to the host
# that created it, and it appears in lsusb, in /dev/bus/usb, and to libusb as
# an ordinary device. The descriptors come from configfs, so the vendor ID,
# product ID and interface count are ours to choose -- which is what makes the
# device-identification logic testable at all.
#
# The gadget's function is Loopback (usb_f_ss_lb), chosen because it is the
# one in-tree function that just presents bulk endpoints without pretending to
# be storage. Nothing here touches a block device.
#
# Usage:  usb_gadget_emulator.sh check
#         usb_gadget_emulator.sh up <vid> <pid> <interfaces>
#         usb_gadget_emulator.sh down
