...
// uniqueKey() is how the picker tells two attached devices apart. Rpiboot and
// fastboot devices have no stable /dev node to key on -- they are identified
// by USB port path -- so these two arms build the key by walking that path.
// If they collided, two Pis plugged into different ports would share one row.
TEST_CASE("A device key distinguishes two Pis on different USB ports", "[models][drivelist]")
{
    SECTION("rpiboot keys on the chip and the port path")
    {
        Drivelist::DeviceDescriptor a;
        a.isRpiboot = true;
        a.rpibootChipName = "bcm2712";
        a.usbPortPath = {1, 2};

        Drivelist::DeviceDescriptor b = a;
        b.usbPortPath = {1, 3};

        CHECK(a.uniqueKey() == "rpiboot:bcm2712:1.2");
        CHECK(b.uniqueKey() == "rpiboot:bcm2712:1.3");
        CHECK(a.uniqueKey() != b.uniqueKey());
    }

    SECTION("fastboot keys on the port path and the block device")
    {
        Drivelist::DeviceDescriptor a;
        a.isFastbootStorage = true;
        a.fastbootPortPath = {2, 4, 1};
        a.fastbootBlockDevice = "mmcblk0";

        Drivelist::DeviceDescriptor b = a;
        b.fastbootBlockDevice = "nvme0n1";

        CHECK(a.uniqueKey() == "fastboot:2.4.1:mmcblk0");
        CHECK(b.uniqueKey() == "fastboot:2.4.1:nvme0n1");
        CHECK(a.uniqueKey() != b.uniqueKey());
    }

    SECTION("an ordinary drive keys on its node and size")
    {
        Drivelist::DeviceDescriptor d;
        d.device = "/dev/sda";
        d.size = 1024;
        CHECK(d.uniqueKey() == "/dev/sda:1024");

        d.isReadOnly = true;
        CHECK(d.uniqueKey() == "/dev/sda:1024ro");
    }
}

// A device description comes off the bus and goes straight into the UI, so a
// label can carry whatever the device claims. Tag characters and variation
// selectors are invisible when rendered but survive a copy, which is enough
// to hide content inside a name the user is reading to choose a disk.
TEST_CASE("Invisible characters are stripped from a device label", "[models][drivelist]")
{
    SECTION("tag characters are dropped")
    {
        // U+E0041 (TAG LATIN CAPITAL LETTER A), UTF-8 f3 a0 81 81.
        const std::string label = "Kingston\xF3\xA0\x81\x81 DataTraveler";
        const std::string clean = Drivelist::sanitizeForDisplay(label);
        CHECK(clean == "Kingston DataTraveler");
    }

    SECTION("variation selectors are dropped")
    {
        // U+FE0F (VARIATION SELECTOR-16), UTF-8 ef b8 8f.
        const std::string label = "SanDisk\xEF\xB8\x8F Ultra";
        const std::string clean = Drivelist::sanitizeForDisplay(label);
        CHECK(clean == "SanDisk Ultra");
    }

    SECTION("ordinary text is left alone")
    {
        CHECK(Drivelist::sanitizeForDisplay("Generic USB Disk") == "Generic USB Disk");
    }
}
