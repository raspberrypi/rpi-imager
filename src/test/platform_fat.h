 ...
 *
 * macOS has its own formatter, newfs_msdos, but it will only work on a device:
 * pointed at a plain file it fails asking for a partition offset. hdiutil will
 * attach a raw image as one, and -- unlike a Linux loop device -- without
 * privileges. So attach, format, detach, and the file is left holding the same
 * filesystem mkfs.vfat would have written.
 */
