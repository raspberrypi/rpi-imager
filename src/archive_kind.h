...
//
//   .img              RAW, no filter        the image itself
//   .iso              ISO9660, no filter    a filesystem image: also itself
//   .img.xz           RAW, xz filter        must be decompressed
//   .zip, .tar.gz     ZIP or TAR            must be unpacked
//
// Note the filter test rather than a list of extensions: a compressed image
// reports RAW like a plain one does, and the decompression filter is the only
