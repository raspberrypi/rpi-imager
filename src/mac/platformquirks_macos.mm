...
namespace {
    ...
    // Mark an operation complete. The caller is pumping its own run loop and
    // sees `completed` on the next turn, so there is nobody to signal.
    ...
    //
    // The session runs on whichever thread called, on that thread's own run
    // loop. It used to depend on the thread it was NOT called from: a worker
    // handed the DiskArbitration work to the main queue with dispatch_sync and
    // waited on a semaphore, which is only safe while something is draining
    // that queue. An application running an event loop does; a process whose
    // main thread is anywhere else does not, and the two threads then wait on
    // each other forever. Two test binaries deadlocked exactly so -- the main
    // thread inside ~ImageWriter, in QThread::wait() for the very worker that
    // was waiting for it -- and the same shape is reachable in the app itself
    // any time an unmount overlaps a main thread that is not pumping, shutdown
    // being the obvious one. Nothing needs the main thread here: the callbacks
    // set two fields on a stack context, and a worker has no Qt event loop to
    // re-enter.
    PlatformQuirks::DiskResult runDiskOperationOnCurrentRunLoop(const char* device,
    ...
    PlatformQuirks::DiskResult runDiskOperation(const char* device,
                                                 DADiskUnmountCallback callback,
                                                 int maxRetries = 12) {
        return runDiskOperationOnCurrentRunLoop(device, callback, maxRetries);
    }
}
