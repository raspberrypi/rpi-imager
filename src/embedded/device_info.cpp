        ...
        if (f.open(QFile::ReadOnly | QFile::Text)) {
            ...
            while (!s.atEnd()) {
                ...
                if (line.startsWith("Revision")) {
                    ...
                    if (!ok) { qDebug() << "Revision Code Not OK!"; }

                    // New Style Revision Code
                    // Device type is 8 bits, stored in bits 4-11
                    const uint32_t MASK = 0xFF << 4;
                    uint32_t isolated_bits = value_int & MASK;
                    uint32_t extracted_value = isolated_bits >> 4;

                    // convert final value back to HEX
                    std::stringstream ss;
                    ss << std::hex << extracted_value;
                    QString extracted_hex_string = QString::fromStdString(ss.str());
                    _device_code = extracted_hex_string;

                    // Make a Human Readable Equivalent of this device_code
