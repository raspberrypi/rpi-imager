...
protected:
    // Cache management
    //
    // Protected rather than private so a test can reach the cache the way the
    // application does. Whether pressing Write re-downloads the image, or
    // writes a cached copy that failed its integrity check, is decided from
    // this object's state at the moment startWrite() runs -- and the states
    // that matter are the ones the *background* verifier leaves behind before
    // the user gets there. Nothing else can put it in them, and nothing else
    // can tell when it has. Same reason DownloadThread::_file is protected.
